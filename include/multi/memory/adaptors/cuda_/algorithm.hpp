...
$CXXX $CXXFLAGS $0 - o $0x - lcudart - lboost_unit_test_framework - lboost_timer&& $0x&& rm $0x;
exit
#endif
...
#include <complex>  //TODO remove, handle complex_fix instead?
	...
	auto copy_n(iterator<T1, 1, ptr<TT1>> first, Size count, iterator<std::complex<T2>, 1, ptr<std::complex<T2>>> result) {
		...
		if(stride(first) == 1 and stride(result) == 1)
			copy_n(base(first), count, base(result));
		else
			assert(0);
		return result + count;
	}
	...
	array_iterator<T2, 1, ptr<Q2>> copy_n(
		...
		array_iterator<T1, 1, ::thrust::device_ptr<Q1>> first;
		std::memcpy((void*)&first, (void const*)&first_, sizeof(first_));
		array_iterator<T2, 1, ::thrust::device_ptr<Q2>> result;
		std::memcpy((void*)&result, (void const*)&result_, sizeof(first_));
	...
	copy_n(
		...
		array_iterator<T1, 2, ::thrust::device_ptr<Q1>> first;
		std::memcpy((void*)&first, (void const*)&first_, sizeof(first_));
	...
	copy_n(
		...
		array_iterator<T1, 3, ::thrust::device_ptr<Q1>> first;
		std::memcpy((void*)&first, (void const*)&first_, sizeof(first_));
		array_iterator<T2, 3, ::thrust::device_ptr<Q2>> result;
		std::memcpy((void*)&result, (void const*)&result_, sizeof(first_));
		static_assert(sizeof(first) == sizeof(first_));
		static_assert(sizeof(result) == sizeof(result_));
		assert(first->extensions() == result->extensions());
	...
	copy_n(
		...
		array_iterator<T1, 4, ::thrust::device_ptr<Q1>> first;
		std::memcpy((void*)&first, (void const*)&first_, sizeof(first_));
		array_iterator<T2, 4, ::thrust::device_ptr<Q2>> result;
		std::memcpy((void*)&result, (void const*)&result_, sizeof(first_));
		static_assert(sizeof(first) == sizeof(first_));
		static_assert(sizeof(result) == sizeof(result_));
		assert(first->extensions() == result->extensions());
	...
	copy_n(
		...
		array_iterator<T1, 5, ::thrust::device_ptr<Q1>> first;
		std::memcpy((void*)&first, (void const*)&first_, sizeof(first_));
		array_iterator<T2, 5, ::thrust::device_ptr<Q2>> result;
		std::memcpy((void*)&result, (void const*)&result_, sizeof(first_));
		static_assert(sizeof(first) == sizeof(first_));
		static_assert(sizeof(result) == sizeof(result_));
		assert(first->extensions() == result->extensions());
...
#define ENABLE_IF class = std::enable_if_t
	...
	// template<class T1, typename Size, class T2, std::enable_if_t<std::is_same<std::decay_t<T1>, T2>{},int> =0>
	inline managed::ptr<std::complex<double>> copy_n(managed::ptr<double> /*first*/, std::size_t /*count*/, managed::ptr<std::complex<double>> result) {
		assert(0);
		return result;
	}
