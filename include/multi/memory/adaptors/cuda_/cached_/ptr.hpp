...
template<class T, class S> array_wrapper<T> const make_array(T* t, S s);
}  // namespace serialization
}  // namespace boost
...
struct ptr : cuda::ptr<T, RawPtr> {
 ...
 public:
	...
	constexpr ptr& operator+=(typename ptr::difference_type n) {
		(this->rp_) += n;
		return *this;
	}  // remove
	constexpr ptr& operator-=(typename ptr::difference_type n) HD {
		(this->rp_) -= n;
		return *this;
	}  // remove
...
template<class T, class S> boost::serialization::array_wrapper<T> const make_array(ptr<T> t, S s) {
...
int main() {
	...
	f();
	using T = double;
	static_assert(sizeof(cuda::cached::ptr<T>) == sizeof(T*), "!");
