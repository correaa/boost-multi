...
$CXXX $CXXFLAGS $0 - o $0x - lcudart - lboost_unit_test_framework&& $0x&& rm $0x;
exit
#endif
...
#include <cstddef>
	...
	class allocator {  //: protected allocation_counter {
	 ...
	 public:
		...
		[[deprecated("cuda slow")]] void construct(P p, Args&&... args) = delete; /*{
		...
		template<class P>
		[[deprecated("cuda slow")]] void destroy(P p) {
...
//	DECLRETURN(adl_uninitialized_copy(first, count, d_first))
	...
	class allocator<std::max_align_t> {  //: allocation_counter{
	 public:
		...
		template<class P, class... Args>
		void construct(/*[[maybe_unused]]*/ P p, Args&&...) {
			(void)p;
			assert(0);
		}  // TODO investigate who is calling this
		template<class P>
