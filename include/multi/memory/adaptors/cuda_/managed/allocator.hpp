...
$CXXX $CXXFLAGS $0 - o $0x - lcudart&& $0x&& rm $0x;
exit
#endif
	 ...
	 public:
		...
		pointer allocate(typename allocator::size_type n) {
			...
			if(PrefetchDevice::value != -99)
				if(cudaMemPrefetchAsync(raw_pointer_cast(ret), n * sizeof(T), PrefetchDevice::value) != cudaSuccess)
					throw std::runtime_error{"cannot prefetch for some reason"};
		...
		pointer allocate(typename allocator::size_type n, const_void_pointer hint) {
			...
			if(not hint) {
				if(cudaMemPrefetchAsync(raw_pointer_cast(ret), n * sizeof(T), /*device*/ 0) != cudaSuccess)
					throw std::runtime_error{"cannot prefetch"};
				return ret;
			}
			cudaPointerAttributes attr;
			if(cudaPointerGetAttributes(&attr, raw_pointer_cast(hint)) != cudaSuccess)
				throw std::runtime_error{"cannot use attributes for hint"};
			switch(attr.type) {
			case cudaMemoryTypeUnregistered: {  // std::cout<< n <<" cudaMemoryTypeUnregistered"<< attr.device <<" "<< attr.device <<" cpuid:"<< cudaCpuDeviceId <<std::endl;
				if(cudaMemPrefetchAsync(raw_pointer_cast(ret), n * sizeof(T), cudaCpuDeviceId) != cudaSuccess)
					throw std::runtime_error{"could not prefetch in managed memory"};
				return ret;
			}
			...
			case cudaMemoryTypeDevice: {  // std::cout<< n <<" cudaMemoryTypeDevice "<< attributes.device <<" "<< attributes.device<<std::endl;
				if(cudaMemPrefetchAsync(raw_pointer_cast(ret), n * sizeof(T), attr.device) != cudaSuccess)
					throw std::runtime_error{"could not prefetch in managed memory"};
				return ret;
			}
			case cudaMemoryTypeManaged: {  // std::cout<< n <<" cudaMemoryTypeManaged "<< attr.device <<" "<< attr.device <<std::endl;
				if(cudaMemPrefetchAsync(raw_pointer_cast(ret), n * sizeof(T), attr.device /*0?*/) != cudaSuccess)
					throw std::runtime_error{"could not prefetch in managed memory"};
				return ret;
			}
			}
