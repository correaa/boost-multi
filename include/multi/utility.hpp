...
struct move_ptr : private std::move_iterator<Ptr> {
	...
	HD constexpr auto operator+=(difference_type n) -> move_ptr& {
	...
	HD constexpr auto operator+(difference_type n) const -> move_ptr {
		move_ptr ret{*this};
		ret += n;
		return ret;
	}
...
struct transform_ptr {
	...
	constexpr auto operator+(difference_type n) const -> transform_ptr {
		transform_ptr ret{*this};
		ret += n;
		return ret;
	}
	constexpr auto operator-(difference_type n) const -> transform_ptr {
