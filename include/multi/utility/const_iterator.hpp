...
$CXX $0 - o $0x - DBOOST_TEST_DYN_LINK - lboost_unit_test_framework&& $0x&& rm $0x;
exit
#endif
	 ...
	 public:
		...
		template<class Other, std::enable_if_t<
			                      std::is_same<typename std::iterator_traits<Other>::value_type, typename std::iterator_traits<Iter>::value_type>{} and
				                      ...
				                      not std::is_assignable<typename std::iterator_traits<Other>::reference, typename std::iterator_traits<Iter>::value_type>{},
			                      int> = 0>
...
BOOST_AUTO_TEST_CASE(sematics) {
	...
	*it += 1;
	...
	std::vector<int>::const_iterator cit = multi::make_const_iterator(v.begin());
	(void)cit;
}
