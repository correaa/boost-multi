...
struct archive_traits<Ar, typename std::enable_if_t<std::is_base_of_v<boost::archive::detail::common_oarchive<Ar>, Ar> || std::is_base_of_v<boost::archive::detail::common_iarchive<Ar>, Ar>>> {
	...
	template<class T>        inline static auto make_nvp          (char const* name, T&& value) noexcept -> const nvp<T> {return nvp<T>{name, std::forward<T>(value)};}  // NOLINT(readability-const-return-type) : match original boost declaration
...
struct archive_traits<
	...
	typename std::enable_if_t<
		...
		|| std::is_base_of_v<cereal::InputArchive <Ar, 0>, Ar> or std::is_base_of_v<cereal:: InputArchive<Ar, 1>, Ar>
	>
> {
