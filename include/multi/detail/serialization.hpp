...
struct archive_traits<Ar, typename std::enable_if<std::is_base_of_v<boost::archive::detail::common_oarchive<Ar>, Ar> || std::is_base_of_v<boost::archive::detail::common_iarchive<Ar>, Ar>>::type> {
	...
	template<class T> inline static auto make_nvp(char const* name, T& value) noexcept -> nvp<T> const { return nvp<T>{name, value}; }  // NOLINT(readability-const-return-type) : original boost declaration
	template<class T> inline static auto make_nvp(char const* name, T&& value) noexcept -> nvp<T> const { return nvp<T>{name, value}; }  // NOLINT(readability-const-return-type) : original boost declaration
	...
	template<class T = void> inline static auto make_binary_object(void const* first, std::size_t size) noexcept -> typename binary_object_t<T>::type const { return typename binary_object_t<T>::type(first, size); }  // if you get an error here you need to eventually `#include<boost/serialization/binary_object.hpp>`// NOLINT(readability-const-return-type,clang-diagnostic-ignored-qualifiers) : original boost declaration
};
...
struct archive_traits<
	...
	typename std::enable_if<
		std::is_base_of_v<cereal::OutputArchive<Ar, 0>, Ar> or std::is_base_of_v<cereal::OutputArchive<Ar, 1>, Ar> or std::is_base_of_v<cereal::InputArchive<Ar, 0>, Ar> or std::is_base_of_v<cereal::InputArchive<Ar, 1>, Ar>>::type> {
