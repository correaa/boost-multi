...
(echo "#include\"" $0 "\"" > $0.cpp) && nvcc-- compiler - options - std = c++ 14, -Wall, -Wextra, -Wpedantic`#, -Wfatal - errors` - D_TEST_BOOST_MULTI_DETAIL_MEMORY_CUDA_PTR $0.cpp - o $0x&& $0x&& rm $0x $0.cpp;
exit
#endif
...
#include <type_traits>  // is_const
	...
	class ptr {
	 ...
	 public:
		...
		ptr& operator+=(typename ptr::difference_type n) {
			impl_ += n;
			return *this;
		}
		ptr& operator-=(typename ptr::difference_type n) {
	...
	struct ref : private ptr<T> {
	 ...
	 private:
		ref& move_assign(ref&& other, std::true_type) & {
			cudaError_t s = cudaMemcpy(this->impl_, other.impl_, sizeof(T), cudaMemcpyDeviceToDevice);
			assert(s == cudaSuccess);
			return *this;
		}
	 ...
	 public:
		...
		auto operator+(Other&& o) && -> decltype(std::move(*this).skeleton() + std::forward<Other>(o)) {
		...
		ref&& operator=(value_type const& t) && {
			make_overload(
				...
				[&](std::false_type) {
					std::array<char, sizeof(T)> buff;  // char buff[sizeof(T)];
					cudaError_t                 s1 = cudaMemcpy(buff.data(), this->impl_, buff.size(), cudaMemcpyDeviceToHost);
					assert(s1 == cudaSuccess);
					reinterpret_cast<T&>(buff) = t;
					cudaError_t s2             = cudaMemcpy(this->impl_, buff.data(), buff.size(), cudaMemcpyHostToDevice);
		...
		decltype(auto) operator==(ref<Other>&& other) && {
			...
			cudaError_t                 s1 = cudaMemcpy(buff1.data(), this->impl_, buff1.size(), cudaMemcpyDeviceToHost);
			assert(s1 == cudaSuccess);
			std::array<char, sizeof(Other)> buff2;  // char buff2[sizeof(Other)];
			cudaError_t                     s2 = cudaMemcpy(buff2.data(), other.impl_, buff2.size(), cudaMemcpyDeviceToHost);
			assert(s2 == cudaSuccess);
		...
		operator T() && {
			...
			std::array<char, sizeof(T)> buff;  // char buff[sizeof(T)];
			cudaError_t                 s = cudaMemcpy(buff.data(), this->impl_, buff.size(), cudaMemcpyDeviceToHost);
			assert(s == cudaSuccess);
