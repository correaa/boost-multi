...
auto alloc_uninitialized_copy(Alloc& alloc, InputIt first, InputIt last, MIt dest) -> MIt {
	...
	try {
		for(; first != last; ++first, ++current) {
			alloc.construct(to_address(current), *first);
...
template<class Alloc>
struct is_allocator<Alloc, std::void_t<decltype(std::declval<Alloc const&>() == Alloc{std::declval<Alloc const&>()},
