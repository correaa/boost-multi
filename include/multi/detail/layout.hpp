...
struct extensions_t {
 ...
 public:
	...
	constexpr auto next_canonical(index& idx, Indices&... rest) const -> bool {  // NOLINT(google-runtime-references) idx is mutated
		if(extensions_t<D - 1>{tail(this->base())}.next_canonical(rest...)) {
			++idx;
		}
	...
	constexpr auto prev_canonical(index& idx, Indices&... rest) const -> bool {  // NOLINT(google-runtime-references) idx is mutated
		if(extensions_t<D - 1>{tail(this->base())}.prev_canonical(rest...)) {
			--idx;
		}
...
template<> struct extensions_t<1> {
 ...
 public:
	...
	constexpr /*implicit*/ extensions_t(multi::size_t size) : impl_(multi::index_extension{0, size}) {}  // NOLINT(google-explicit-constructor,hicpp-explicit-conversions)
...
: multi::equality_comparable<layout_t<D, SSize>> {
 ...
 public:
	layout_t() = default;
	HD constexpr explicit layout_t(extensions_type const& extensions) : sub_{
		                                                                    std::apply(
 ...
 public:
	...
	constexpr auto reindex(index idx) -> layout_t& {
		offset_ = idx * stride_;
		return *this;
	}
	template<class... Indices>
	constexpr auto reindex(index idx, Indices... rest) -> layout_t& {
		reindex(idx).rotate().reindex(rest...).unrotate();
		return *this;
	}
	...
	constexpr auto        extensions() const { return extensions_type{
                                                                                                                                                                                                      tuple{extension(), sub_.extensions().base()}
	...
	constexpr auto extension(dimensionality_type dim) const {
