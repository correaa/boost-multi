...
template<class T0, class... Ts> class tuple<T0, Ts...> {  // NOLINT(cppcoreguidelines-special-member-functions,hicpp-special-member-functions)
 ...
 public:
	...
	constexpr explicit tuple(T0 t0, Ts...        ts ) : head_{std::move(t0)}, tail_{std::move(ts)...} {}
...
template<class T0> class tuple<T0> : private tuple<> {  // NOLINT(cppcoreguidelines-special-member-functions,hicpp-special-member-functions)
 ...
 public:
	...
	constexpr auto tail() const& -> tuple<> const& {return static_cast<tuple<> const&>(*this);}
	constexpr auto tail()     && -> tuple<>     && {return static_cast<tuple<>     &&>(*this);}
	constexpr auto tail()      & -> tuple<>      & {return static_cast<tuple<>      &>(*this);}
