...
int main() {
	{
		...
		for(int i = 0; i != 3; ++i) {
			cout << "pass " << i << std::endl;
			{
				...
				multi::array<double, 2, multi::stack_allocator<double, std::allocator<char>, 32>> A({2, 10}, &buf);
				assert(is_aligned(alignof(double), &A[1][3]));
				multi::array<double, 2, multi::stack_allocator<double, std::allocator<char>, 32>> B({3, 10}, &buf);
				assert(is_aligned(alignof(double), &B[2][3]));
				multi::array<double, 2, multi::stack_allocator<double, std::allocator<char>, 32>> C({4, 10}, &buf);
				assert(is_aligned(alignof(double), &C[3][3]));
				std::vector<int, multi::stack_allocator<int, std::allocator<char>, 32>> v(3, &buf);
				assert(is_aligned(alignof(int), &v[1]));

				for(int j = 0; j != 100; ++j) {
					multi::array<double, 2, multi::stack_allocator<double, std::allocator<char>, 32>> D({4, 10}, &buf);
					assert(is_aligned(alignof(double), &D[2][1]));
					multi::array<float, 2, multi::stack_allocator<>> E({4, 10}, &buf);
					assert(is_aligned(alignof(float), &E[3][3]));
				}

				multi::array<double, 2, multi::stack_allocator<>> F({4, 10}, &buf);
				assert(is_aligned(alignof(double), &F[2][1]));
			}
			cout << "  size: " << buf.size()
			     << "\n  hits: " << buf.hits()
			     << "\n  misses " << buf.misses()
