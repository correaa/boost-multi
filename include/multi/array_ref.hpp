...
struct const_subarray : private Layout {  // cppcheck-nosuppress syntaxError ; false positive in cppcheck
	...
	friend constexpr auto stride(const_subarray const& self) noexcept -> stride_type { return self.stride(); }

	// TODO(correaa) [[deprecated("use member syntax for non-salient properties")]]
	friend constexpr auto strides(const_subarray const& self) noexcept -> strides_type { return self.strides(); }
 ...
 protected:
	template<
		class ArrayTypes,
		typename = std::enable_if_t<!std::is_base_of<const_subarray, std::decay_t<ArrayTypes>>{}>, decltype(multi::detail::explicit_cast<element_ptr>(std::declval<ArrayTypes const&>().base_))* = nullptr>
...
: boost::multi::iterator_facade<
	  ...
	  Ref const&, typename Layout::difference_type> {  //, boost::multi::totally_ordered2<subarray_ptr<Ref, Layout>, void>
 private:
	mutable Ref ref_;
 ...
 public:
	HD constexpr auto operator+=(difference_type n) -> subarray_ptr& {
		advance(n);
		return *this;
	}
};
...
, multi::totally_ordered2<array_iterator<Element, D, ElementPtr>, void> {
	...
	HD constexpr auto operator+(difference_type n) const -> array_iterator {
...
: boost::multi::random_accessable<elements_iterator_t<Pointer, LayoutType>, typename std::iterator_traits<Pointer>::difference_type, typename std::iterator_traits<Pointer>::reference> {
 ...
 public:
	...
	HD constexpr auto operator+(difference_type n) const -> elements_iterator_t {
		auto ret{*this};
		ret += n;
		return ret;
...
struct elements_range_t {
 ...
 public:
	...
	[[nodiscard]] constexpr auto empty() const -> bool { return l_.empty(); }
 ...
 public:
	...
	auto operator=(OtherElementRange&& other) & -> elements_range_t& {
		assert(size() == other.size());
		if(!is_empty()) {
...
struct subarray : const_subarray<T, D, ElementPtr, Layout> {
 ...
 public:
	...
	MULTI_FRIEND_CONSTEXPR HD auto operator~(subarray const& self) -> basic_const_array { return self.transposed(); }
	MULTI_FRIEND_CONSTEXPR HD auto operator~(subarray& self) -> subarray { return self.transposed(); }
	MULTI_FRIEND_CONSTEXPR HD auto operator~(subarray&& self) -> subarray { return std::move(self).transposed(); }
 ...
 public:
	...
	constexpr auto operator=(subarray<TT, D, As...> const& other) && -> subarray& {
		operator=(other);
		return *this;
	}
	...
	constexpr auto operator=(subarray const& other) && -> subarray& {  // lints(cppcoreguidelines-c-copy-assignment-signature,misc-unconventional-assign-operator)
		if(this == std::addressof(other)) {
			return *this;
		}  // lints(cert-oop54-cpp)
		operator=(other);
 ...
 public:
	...
	template<class T2, class P2 = typename std::pointer_traits<element_ptr>::template rebind<T2>, std::enable_if_t<!std::is_const_v<typename std::pointer_traits<P2>::element_type>, int> = 0>
	[[deprecated("violates constness")]] constexpr auto static_array_cast() const& {  // name taken from std::static_pointer_cast
		return subarray<T2, D, P2>(this->layout(), static_cast<P2>(this->base_));  // TODO(correaa) might violate constness
	}
 ...
 public:
	...
	constexpr auto reinterpret_array_cast(multi::size_type count) & -> subarray<std::decay_t<T2>, D + 1, P2> {
		...
		return {
			layout_t<D + 1>{this->layout().scale(sizeof(T), sizeof(T2)), 1, 0, count}
				.rotate(), // NOLINT(bugprone-sizeof-expression) T and T2 are size compatible (see static_assert above)
...
, multi::totally_ordered2<array_iterator<Element, 1, Ptr>, void> {
 ...
 public:
	...
	[[deprecated("use base() for iterator")]] constexpr auto data() const -> element_ptr { return data_; }
...
: const_subarray<T, 0, ElementPtr, Layout> {
	...
	constexpr auto operator=(Range0 const& rng) & -> subarray& {
...
, const_subarray<T, 1, ElementPtr, Layout> {
 ...
 public:
	...
	HD constexpr void assign(std::initializer_list<typename subarray::value_type> values) const {
		assert(values.size() == static_cast<std::size_t>(this->size()));
	...
	constexpr void assign(It first, It last) & {
		...
		(void)last;  // NOLINT(cppcoreguidelines-pro-bounds-array-to-pointer-decay,hicpp-no-array-decay) : normal in a constexpr function
		assign(first);
	}
	template<class It>
 ...
 public:
	...
	template<class Tuple, std::enable_if_t<(std::tuple_size<Tuple>::value > 1), int> = 0> HD constexpr auto  operator[](Tuple const& indices) const& -> decltype(operator[](std::get<0>(indices))[detail::tuple_tail(indices)]) {
                                                                                                                                                                                                      return operator[](std::get<0>(indices))[detail::tuple_tail(indices)];
	}

	HD constexpr auto elements_at(size_type idx) const& -> decltype(auto) {
		assert(idx < this->num_elements());
		return operator[](idx);
	}
	HD constexpr auto elements_at(size_type idx) && -> decltype(auto) {
		assert(idx < this->num_elements());
		return operator[](idx);
	}
	HD constexpr auto elements_at(size_type idx) & -> decltype(auto) {
 ...
 public:
	...
	template<class... Args>
	constexpr auto operator()(Args&&... args) & -> decltype(paren(*this, std::forward<Args>(args)...)) {
		return paren(*this, std::forward<Args>(args)...);
	}

	template<class... Args>
	constexpr auto operator()(Args&&... args) && -> decltype(paren(std::move(*this), std::forward<Args>(args)...)) {
		return paren(std::move(*this), std::forward<Args>(args)...);
	}

	template<class... Args>
	constexpr auto operator()(Args&&... args) const& -> decltype(paren(*this, std::forward<Args>(args)...)) {
		return paren(*this, std::forward<Args>(args)...);
	}
 ...
 private:
	...
	static constexpr auto lexicographical_compare(A1 const& self, A2 const& other) -> bool {  // NOLINT(readability-suspicious-call-argument)
		if(extension(self).first() > extension(other).first()) {
			return true;
		}
		if(extension(self).first() < extension(other).first()) {
			return false;
...
: subarray<T, D, ElementPtr> {
 ...
 protected:
	[[deprecated("references are not copyable, use auto&&")]] array_ref(array_ref const&) = default;  // don't try to use `auto` for references, use `auto&&` or explicit value type
 ...
 public:
	...
	template<class Tconst = typename array_ref::element_type const, std::enable_if_t<std::is_convertible_v<typename array_ref::element_const_ptr, Tconst*> and D == 1, int> = 0>
...
struct array_ptr
: subarray_ptr<subarray<T, D, Ptr>, typename array_ref<T, D, Ptr>::layout_t> {
...
constexpr auto operator/(RandomAccessIterator data, multi::extensions_t<D> extensions)
	-> multi::array_ptr<typename std::iterator_traits<RandomAccessIterator>::value_type, D, RandomAccessIterator> { return {data, extensions}; }
