...
constexpr auto data_cast(T* p) {
	if constexpr(is_s<T>{}) {
		...
		return reinterpret_cast<DoubleComplex*>(p);
	}
}
...
constexpr auto data_cast(T const* p) {
	if constexpr(is_s<T>{}) {
		...
		return reinterpret_cast<double const*>(p);
	} else if constexpr(is_c<T>{}) {
		return reinterpret_cast<Complex const*>(p);
	} else if constexpr(is_z<T>{}) {
		return reinterpret_cast<DoubleComplex const*>(p);
	}
}
...
class context : private std::unique_ptr<typename std::pointer_traits<hicu(blasHandle_t)>::element_type, decltype(&hicu(blasDestroy))> {
	...
	hicup(Stream_t) stream() const {
		hicup(Stream_t) streamId;
		cuda::cublas::call<hicu(blasGetStream)>(this->get(), &streamId);
		return streamId;
	}
 ...
 public:
	...
	void synchronize() const {
		...
		auto s = stream();
		if(s != 0) {
			throw std::logic_error("CUBLAS stream expected to be zero");
		}
		hicup(Error_t) e = hicup(StreamSynchronize)(s);
		if(e != hicup(Success)) {
			throw std::runtime_error{"cannot synchronize stream in cublas context"};
		}
	}
	...
	void swap(ssize_t n, XP x, ssize_t incx, YP y, ssize_t incy) const {
		if(is_s<X>{}) {
			sync_call<hicu(blasSswap)>(n, (float*)raw_pointer_cast(x), incx, (float*)raw_pointer_cast(y), incy);
		}
		if(is_d<X>{}) {
			sync_call<hicu(blasDswap)>(n, (double*)raw_pointer_cast(x), incx, (double*)raw_pointer_cast(y), incy);
		}
		if(is_c<X>{}) {
			sync_call<hicu(blasCswap)>(n, (Complex*)raw_pointer_cast(x), incx, (Complex*)raw_pointer_cast(y), incy);
		}
		if(is_z<X>{}) {
			sync_call<hicu(blasZswap)>(n, (DoubleComplex*)raw_pointer_cast(x), incx, (DoubleComplex*)raw_pointer_cast(y), incy);
		}
	}
	...
	void copy(ssize_t n, XP x, ssize_t incx, YP y, ssize_t incy) const {
		if(is_s<X>{}) {
			sync_call<hicu(blasScopy)>(n, (float const*)raw_pointer_cast(x), incx, (float*)raw_pointer_cast(y), incy);
		}
		if(is_d<X>{}) {
			sync_call<hicu(blasDcopy)>(n, (double const*)raw_pointer_cast(x), incx, (double*)raw_pointer_cast(y), incy);
		}
		if(is_c<X>{}) {
			sync_call<hicu(blasCcopy)>(n, (Complex const*)raw_pointer_cast(x), incx, (Complex*)raw_pointer_cast(y), incy);
		}
		if(is_z<X>{}) {
			sync_call<hicu(blasZcopy)>(n, (DoubleComplex const*)raw_pointer_cast(x), incx, (DoubleComplex*)raw_pointer_cast(y), incy);
		}
	}
	...
	void scal(ssize_t n, ALPHA const& alpha, XP x, ssize_t incx) const {
		if(is_s<X>{}) {
			sync_call<hicu(blasSscal)>(n, (float const*)alpha, (float*)::thrust::raw_pointer_cast(x), incx);
		}
		if(is_d<X>{}) {
			sync_call<hicu(blasDscal)>(n, (double const*)alpha, (double*)::thrust::raw_pointer_cast(x), incx);
		}
		if(is_c<X>{}) {
			sync_call<hicu(blasCscal)>(n, (Complex const*)alpha, (Complex*)::thrust::raw_pointer_cast(x), incx);
		}
		if(is_z<X>{}) {
			sync_call<hicu(blasZscal)>(n, (DoubleComplex const*)alpha, (DoubleComplex*)::thrust::raw_pointer_cast(x), incx);
		}
	}
	...
	void axpy(ssize_t n, ALPHA const* alpha, XP x, ssize_t incx, YP y, ssize_t incy) {
		if(is_d<X>{}) {
			sync_call<hicu(blasDaxpy)>(n, (double const*)alpha, (double const*)raw_pointer_cast(x), incx, (double*)raw_pointer_cast(y), incy);
		}
		if(is_z<X>{}) {
			sync_call<hicu(blasZaxpy)>(n, (DoubleComplex const*)alpha, (DoubleComplex const*)raw_pointer_cast(x), incx, (DoubleComplex*)raw_pointer_cast(y), incy);
		}
	}
	...
	auto gemv(char transA, ssize_t m, ssize_t n, ALPHA const* alpha, AAP aa, ssize_t lda, XXP xx, ssize_t incx, BETA const* beta, YYP yy, ssize_t incy) {
		if(is_d<AA>{}) {
			sync_call<hicu(blasDgemv)>(operation{transA}, m, n, (double const*)alpha, (double const*)::thrust::raw_pointer_cast(aa), lda, (double const*)::thrust::raw_pointer_cast(xx), incx, (double const*)beta, (double*)::thrust::raw_pointer_cast(yy), incy);
		}
		if(is_z<AA>{}) {
			sync_call<hicu(blasZgemv)>(operation{transA}, m, n, (DoubleComplex const*)alpha, (DoubleComplex const*)::thrust::raw_pointer_cast(aa), lda, (DoubleComplex const*)::thrust::raw_pointer_cast(xx), incx, (DoubleComplex const*)beta, (DoubleComplex*)::thrust::raw_pointer_cast(yy), incy);
		}
	}
	...
	void gemm(char transA, char transB, ssize_t m, ssize_t n, ssize_t k, ALPHA const* alpha, AAP aa, ssize_t lda, BBP bb, ssize_t ldb, BETA const* beta, CCP cc, ssize_t ldc) {
		...
		if(is_d<AA>{}) {
			sync_call<hicu(blasDgemm)>(cuda::cublas::operation{transA}, cuda::cublas::operation{transB}, m, n, k, (double const*)alpha, (double const*)::thrust::raw_pointer_cast(aa), lda, (double const*)::thrust::raw_pointer_cast(bb), ldb, (double const*)beta, (double*)::thrust::raw_pointer_cast(cc), ldc);
		}
		if(is_z<AA>{}) {
			sync_call<hicu(blasZgemm)>(cuda::cublas::operation{transA}, cuda::cublas::operation{transB}, m, n, k, (DoubleComplex const*)alpha, (DoubleComplex const*)::thrust::raw_pointer_cast(aa), lda, (DoubleComplex const*)::thrust::raw_pointer_cast(bb), ldb, (DoubleComplex const*)beta, (DoubleComplex*)::thrust::raw_pointer_cast(cc), ldc);
		}
	}
	...
	template<
		...
		std::enable_if_t<
			is_d<XX>{} and is_d<YY>{} and is_d<RR>{} and is_assignable<RR&, decltype(XX{} * YY{})>{} and
				is_convertible_v<XXP, ::thrust::hicup()::pointer<XX>> and is_convertible_v<YYP, ::thrust::hicup()::pointer<YY>> and is_convertible_v<RRP, RR*>,
			int> = 0>
	void dot(int n, XXP xx, int incx, YYP yy, int incy, RRP rr) {
		hicu(blasPointerMode_t) mode;
		auto s = hicu(blasGetPointerMode)(get(), &mode);
		assert(s == HICU(BLAS_STATUS_SUCCESS));
	...
	template<
		...
		std::enable_if_t<
			is_z<XX>{} and is_z<YY>{} and is_z<RR>{} and is_assignable<RR&, decltype(XX{} * YY{})>{} and
				is_convertible_v<XXP, ::thrust::hicup()::pointer<XX>> and is_convertible_v<YYP, ::thrust::hicup()::pointer<YY>> and (is_convertible_v<RRP, ::thrust::hicup()::pointer<RR>> or is_convertible_v<RRP, RR*>),
			int> = 0>
	...
	void asum(int n, XXP xx, int incx, RRP rr) {
		if(is_convertible_v<RRP, ::thrust::hicup()::pointer<RR>>) {
			hicu(blasSetPointerMode)(get(), HICU(BLAS_POINTER_MODE_DEVICE));
		}
		if constexpr(is_convertible_v<RRP, ::thrust::hicup()::pointer<RR>>) {
