...
$CXXX $CXXFLAGS `mpicxx - showme : compile | sed 's/-pthread/ /g'` - I$HOME / prj / alf $0 - o $0x `mpicxx - showme : link | sed 's/-pthread/ /g'` - lfftw3 - lfftw3_mpi&& mpirun - n 4 $0x&& rm $0x;
exit
...
#include "boost/mpi3/process.hpp"
	...
	class gathered_array<T, 2, Alloc> : public array<T, Alloc> {
	 ...
	 public:
		gathered_array(multi::extensions_type_<2> exts, bmpi3::communicator comm, Alloc alloc = {}) : array<T, Alloc>{exts, comm, std::get<0>(exts).size(), std::get<1>(exts).size(), alloc},
		                                                                                              comm_{std::move(comm)} {}
	...
	class scattered_array<T, 2, Alloc> {
	 ...
	 public:
		scattered_array(multi::extensions_type_<2> exts, bmpi3::communicator comm, Alloc alloc = {}) : local_distribution_{exts, comm},
		                                                                                               alloc_{alloc},
