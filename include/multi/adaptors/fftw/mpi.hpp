...
ln - sf $0                      $0.cpp;
mpicxx - g - I$HOME / prj / alf $0.cpp - o $0x - lfftw3 - lfftw3_mpi&& time mpirun - n 4 $0x&& rm $0x $0.cpp;
exit
...
#include <fftw3-mpi.h>
	...
	struct array<T, multi::dimensionality_type{2}, Alloc> {
		...
		local_2d(multi::extensions_t<2> ext, boost::mpi3::communicator const& comm) {
			...
			return {
				count, {{local_0_start, local_0_start + local_n0}, std::get<1>(ext)}
			};
		}
		...
		array(multi::extensions_t<2> ext, bmpi3::communicator comm = mpi3::environment::self(), Alloc alloc = {}) : comm_{std::move(comm)},
			         alloc_{alloc},
		...
		array(array const& other) : comm_{other.comm_},
		                            alloc_{other.alloc_},
		...
		array(array&& other) : comm_{std::move(other.comm_)},
		                       alloc_{std::move(other.alloc_)},
		                       local_count_{std::exchange(other.local_count_, 0)},
		                       local_ptr_{std::exchange(other.local_ptr_, nullptr)},
		                       n0_{multi::layout_t<2>(other.extensions()).size()} {}
		explicit array(multi::array<T, 2> const& other, bmpi3::communicator comm = mpi3::environment::self(), Alloc alloc = {}) : array(other.extensions(), comm, alloc) {
		...
		bool operator==(array<T, 2> const& other) const& {
	...
	array<std::complex<double>, 2> dft_forward(array<std::complex<double>, 2> const& A) {
		array<std::complex<double>, 2> ret(A.extensions());
		dft_forward(A, ret);
		return ret;
	}
