...
(echo "#include\"" $0 "\"" > $0.cpp) && nvcc - ccbin cuda - c++ --compiler - options "-std=c++14" - I$HOME / prj / alf - D_TEST_MULTI_ADAPTORS_THRUST_ALLOCATOR_TRAITS $0.cpp - o $0x&& $0x&& rm $0.cpp $0x;
exit
#endif
...
class allocator_traits<thrust::device_allocator<T>> {
 ...
 private:
	static void construct(std::true_type, Alloc& a, pointer p) {
		cudaError_t s = cudaMemset(raw_pointer_cast(p), 0, sizeof(T));
