...
BOOST_AUTO_TEST_CASE(multi_blas_herk_real) {
	...
	{
		multi::array<double, 2> c({2, 2}, 9999);  // NOLINT(readability-identifier-length) conventional name in BLAS
		blas::herk(1., a, c);
...
BOOST_AUTO_TEST_CASE(multi_blas_herk_complex_identity) {
	...
	multi::array<complex, 2> const a = {  // NOLINT(readability-identifier-length) : conventional one-letter operation BLASs
		{ 1. + 3.*I, 3.- 2.*I, 4.+ 1.*I},
		{ 9. + 1.*I, 7.- 8.*I, 1.- 3.*I}
	};
