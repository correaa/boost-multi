...
auto herk(filling c_side, AA alpha, A2D const& a, BB beta, C2D&& c) -> C2D&& {  // NOLINT(readability-function-cognitive-complexity,readability-identifier-length) 74, BLAS naming
	...
	if constexpr(is_conjugated<C2D>{}) {
		...
		if constexpr(is_conjugated<A2D>{}) {
			...
			if(stride(a) == 1 and stride(c) != 1) {
				herk(c_side == filling::upper ? 'L' : 'U', 'N', size(c), size(rotated(a)), &alpha, base_a, stride(rotated(a)), &beta, base_c, stride(c));
				...
				herk(c_side == filling::upper ? 'U' : 'L', 'C', size(c), size(rotated(a)), &alpha, base_a, stride(a), &beta, base_c, stride(rotated(c)));
			} else if(stride(a) != 1 and stride(c) != 1) {
				herk(c_side == filling::upper ? 'L' : 'U', 'C', size(c), size(rotated(a)), &alpha, base_a, stride(a), &beta, base_c, stride(c));
			} else {
				assert(0);
			}  // NOLINT(cppcoreguidelines-pro-bounds-array-to-pointer-decay,hicpp-no-array-decay)
		} else {
			...
			if(stride(a) != 1 and stride(c) != 1) {
				...
				if(size(a) == 1) {
					herk(c_side == filling::upper ? 'L' : 'U', 'N', size(c), size(rotated(a)), &alpha, base_a, stride(rotated(a)), &beta, base_c, stride(rotated(c)));
				} else {
					assert(0);
				}  // NOLINT(cppcoreguidelines-pro-bounds-array-to-pointer-decay,hicpp-no-array-decay)
			} else if(stride(a) == 1 and stride(c) != 1) {
				...
				herk(c_side == filling::upper ? 'U' : 'L', 'N', size(c), size(rotated(a)), &alpha, base_a, stride(rotated(a)), &beta, base_c, stride(rotated(c)));
			}
