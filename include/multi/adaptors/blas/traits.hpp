...
template<class T> struct is_s : decltype(is_s_aux(std::declval<T>())) {
	using archetype = float;
...
template<class T> struct is_d : decltype(is_d_aux(std::declval<T>())) {
	using archetype = double;
};  // NOLINT(cppcoreguidelines-pro-type-vararg,hicpp-vararg)
...
template<class C> struct is_c : decltype(is_c_aux(std::declval<C>())) {
	using archetype = std::complex<float>;
};  // NOLINT(cppcoreguidelines-pro-type-vararg,hicpp-vararg)
...
template<class Z> struct is_z : decltype(is_z_aux(std::declval<Z>())) {
	using archetype = std::complex<double>;
};  // NOLINT(cppcoreguidelines-pro-type-vararg,hicpp-vararg)
