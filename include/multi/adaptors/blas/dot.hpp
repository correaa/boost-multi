...
auto dot_n(Context&& ctxt, XIt x_first, Size count, YIt y_first, RPtr rp) {
	if constexpr(is_complex<typename XIt::value_type>{}) {
		if constexpr(!is_conjugated<XIt>{} and !is_conjugated<YIt>{}) {
			ctxt->dotu(count, base(x_first), stride(x_first), base(y_first), stride(y_first), rp);
		} else if constexpr(!is_conjugated<XIt>{} and is_conjugated<YIt>{}) {
			ctxt->dotc(count, underlying(base(y_first)), stride(y_first), base(x_first), stride(x_first), rp);
		} else if constexpr(is_conjugated<XIt>{} and !is_conjugated<YIt>{}) {
			ctxt->dotc(count, underlying(base(x_first)), stride(x_first), base(y_first), stride(y_first), rp);
		} else if constexpr(is_conjugated<XIt>{} and is_conjugated<YIt>{}) {
...
template<class Context, class X, class Y> [[nodiscard]] auto dot(Context ctxt, X const& x, Y const& y) {  // NOLINT(readability-identifier-length) BLAS naming
...
template<class X, class Y> [[nodiscard]] auto dot(X const& x, Y const& y) {  // NOLINT(readability-identifier-length) BLAS naming
...
namespace operators {
template<class X1D, class Y1D> [[nodiscard]] auto operator,(X1D const& x, Y1D const& y)  // NOLINT(readability-identifier-length) BLAS naming
	-> decltype(dot(x, y)) {
	return dot(x, y);
}
