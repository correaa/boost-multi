...
struct asum_ptr {
	...
	friend auto copy_n(asum_ptr first, [[maybe_unused]] Size2 count, ItOut d_first)
		-> decltype(blas::asum_n(typename A1D::iterator{}, typename A1D::size_type{}, d_first)) {
		assert(count == 1);
...
template<class A1D>
[[nodiscard]] auto asum(A1D const& x) {  // NOLINT(readability-identifier-length) BLAS naming
	struct ref {
