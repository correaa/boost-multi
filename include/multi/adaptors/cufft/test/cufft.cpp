...
#else
#include<multi/adaptors/hipfft.hpp>
#endif
...
struct watch : private std::chrono::high_resolution_clock{
	...
	watch(std::string label ="") : label_{label}, start_{} {
		{auto const s = cudaDeviceSynchronize(); assert(s == cudaSuccess);}
		start_ = now();
	}
	~watch() {
		{auto const s = cudaDeviceSynchronize(); assert(s == cudaSuccess);}
...
//  std::allocator_traits<boost::multi::cuda::allocator<std::complex<double>>>::const_void_pointer cvp2 = nullptr;
	...
	multi::thrust::device_array<complex, 2> const in_gpu = in_cpu;
	multi::thrust::device_array<complex, 2> fw_gpu(extensions(in_gpu));
	multi::cufft::dft({true, true}, in_gpu, fw_gpu, multi::cufft::forward);

//  BOOST_TEST( std::imag(static_cast<complex>(fw_gpu[3][2]) - fw_cpu[3][2]) == 0.0 );
