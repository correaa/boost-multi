...
(echo "#include\"" $0 "\"" > $0x.cpp) && clang++ `# - DNDEBUG` - O3 - std = c++ 14 - Wall - Wextra - Wpedantic - Wfatal - errors - D_TEST_MULTI_ADAPTORS_LAPACK_CORE - DADD_ $0x.cpp - o $0x.x - lblas - llapack && time $0x.x $ @ && rm - f $0x.x $0x.cpp;
exit
#endif
...
#define xGETRF(T)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        \
	subroutine T##getrf_(                                                                                                                                                                                                                                                                                                                                                                                                 \
		...
		T*          A, /*On entry, the M-by-N matrix to be factored.*/ /*On exit, the factors L and U from the factorization*/                                                                                                                                                                                             \
		...
		integer_out INFO /*= 0:  successful exit*/ /*< 0:  if INFO = -i, the i-th argument had an illegal value*/ /*> 0:  if INFO = i, U(i,i) is exactly zero. The factorization has been completed, but the factor U is exactly singular, and division by zero will occur if it is used to solve a system of equations.*/ \
	)

// http://www.netlib.org/lapack/explore-html/d8/ddc/group__real_g_ecomputational_gaa00bcf4d83a118cb6f0b6619d6ffaa24.html
#define xGETRS(T)                                                                                                                                                                                                                                                                                                                                                                                                                                                                         \
	subroutine T##getrs_(                                                                                                                                                                                                                                                                                                                                                                  \
		character    TRANS, /*Specifies the form of the system of equations:             */ /* = 'N':  A * X = B  (No transpose)                         */ /* = 'T':  A**T* X = B  (Transpose)                          */ /* = 'C':  A**T* X = B  (Conjugate transpose = Transpose)    */ \
		...
		T const*     A, /* The factors L and U from the factorization A = P*L*U      */ /*as computed by SGETRF.                                     */                                                                                                                                     \
		integer      LDA, /*The leading dimension of the array A.  LDA >= max(1,N).    */                                                                                                                                                                                                   \
		integer_cptr IPIV, /*The pivot indices from SGETRF; for 1<=i<=N, row i of the   */ /*matrix was interchanged with row IPIV(i).                  */                                                                                                                                  \
		T*           B, /*On entry, the right hand side matrix B.                    */ /*On exit, the solution matrix X.                            */                                                                                                                                     \
...
#define xpotrf(T)                                                                                                                                                                                                               \
	template<class S> v potrf(char uplo, S n, T* x, S incx, int& info) {                                                         \
		LAPACK(T##potrf)          \
		(uplo, n, x, incx, info); \
	}
...
#define xsyev(T)                                                                                                                                                                                                                                     \
	template<class S> v syev(char jobz, char uplo, S n, T* a, S lda, T* w, T* work, S lwork, int& info) {                                             \
		LAPACK(T##syev)                                \
...
#define xsyevd(T)                                                                                                                                                                                                                                                                                                                                \
	template<class S> v syevd(char jobz, char uplo, S n, T* a, S lda, T* w, T* work, S lwork, int* iwork, S liwork, int& info) {                                                                                                                  \
		if(n <= 1) {                                                                                                                               \
			assert(lwork >= 1);                     \
			assert(liwork >= 1);                    \
		}                                                                                                                                          \
...
#define xheev(T)                                                                                                                                                                                                                                     \
	template<class S> v heev(char jobz, char uplo, S n, T* a, S lda, T* w, T* work, S lwork, int& info) {                                             \
		LAPACK(T##heev)                                \
		(jobz, uplo, n, a, lda, w, work, lwork, info); \
	}
