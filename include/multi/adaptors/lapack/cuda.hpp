...
(echo '#include"' $0 '"' > $0.cpp) && $CXX - Wall - Wextra - Wpedantic - Wfatal - errors - D_TEST_MULTI_ADAPTORS_LAPACK_CUDA $0.cpp - o $0x `pkg - config-- libs blas` - lcudart - lcublas - lcusolver&& $0x&& rm $0x $0.cpp;
exit
#endif
...
#include "../blas/filling.hpp"
	...
	struct error_category : std::error_category {
		...
		static error_category& instance() {
			static cusolver::error_category instance;
			return instance;
		}
	};
	...
	struct context : std::unique_ptr<std::decay_t<decltype(*cusolverDnHandle_t{})>, decltype(&cusolverDnDestroy)> {
		context() : std::unique_ptr<std::decay_t<decltype(*cusolverDnHandle_t{})>, decltype(&cusolverDnDestroy)>{
			            [] {
				            ...
				            auto               s = cusolverDnCreate(&h);
				            assert(CUSOLVER_STATUS_SUCCESS == s and h);
				            return h;
			            }(),
			            &cusolverDnDestroy
		            } {}
	...
	struct cusolverDn<double> {
		...
		template<class... A> static auto syevd(A... a)
			-> decltype(cusolverDnDsyevd(a...)) { return cusolverDnDsyevd(a...); }
	};
	...
	void syev(char jobz, char uplo, S n, PtrT a, S lda, PtrT w, PtrT /*work*/, S /*lwork*/, int& info) {
		...
		if(s != CUSOLVER_STATUS_SUCCESS)
			throw std::system_error{cusolver::make_error_code(static_cast<cusolver::status>(s)), "cannot call cusolver function "};
		//	cudaDeviceSynchronize();
		info = devInfo();
	}
...
namespace std {
template<> struct is_error_code_enum<::boost::multi::cusolver::status> : true_type {};
}  // namespace std
