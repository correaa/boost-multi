...
(echo '#include"' $0 '"' > $0.cpp) && $CXX - D_TEST_MULTI_ADAPTORS_LAPACK_POTRF $0.cpp - o$0x `pkg - config-- libs blas lapack` - lboost_unit_test_framework&& valgrind $0x&& rm $0x $0.cpp;
exit
#endif
...
#include <cassert>
	...
	auto potrf(filling t, A2D const& A)
		...
		auto ret  = decay(A);
		auto last = potrf(t, ret);
		assert(size(last) == size(ret));
		return ret;
	}
	...
	template<class A> auto onrm(A&& a, filling f = filling::upper)
		-> decltype(trsm(flip(f), hermitized(potrf(f, herk(f, a))), std::forward<A>(a))) {
	...
	template<class A, class B> auto onrm(A&& a, B&& buffer, filling f = filling::upper)
		-> decltype(trsm(flip(f), hermitized(potrf(f, herk(f, a, buffer))), std::forward<A>(a))) {
		assert(size(a) <= size(rotated(a)));
