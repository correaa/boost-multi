...
$CXX $0 - o $0x `pkg - config-- libs blas lapack` - lboost_unit_test_framework&& $0x&& rm $0x;
exit
#endif
...
#include <cassert>
	...
	auto syev(blas::filling uplo, Array2D&& a, Array1D&& w, Array1DW&& work)
		...
		if(stride(rotated(a)) == 1)
			...
			assert(0);  // case not contemplated by lapack
		if(info < 0)
	...
	}  // TODO obtain automatic size from lapack info routine
