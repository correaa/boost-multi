...
(echo '#include"' $0 '"' > $0.cpp) && nvcc - x cu-- expt - relaxed - constexpr`#$CXX` $0 - o $0x - Wno - deprecated - declarations - lcudart - lcublas - lcusolver `pkg - config-- libs blas lapack` - DBOOST_TEST_DYN_LINK - lboost_unit_test_framework - DBOOST_LOG_DYN_LINK - lboost_log - lpthread - lboost_system&& $0x&& rm $0x $0.cpp;
exit
#endif
...
M&& randomize(M&& A) {
	std::mt19937 eng{123};
	auto         gen = [&]() { return std::complex<double>{
                                                                                                                                                                                                                         std::uniform_real_distribution<>{-1, 1}
                                                                                                                                                                                                                         (eng), std::uniform_real_distribution<>{-1, 1}
                                                                                                                                                                                                                         (eng)
 }; };
