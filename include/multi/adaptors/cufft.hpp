...
inline void __cufftSafeCall(cufftResult err, char const* file, int const line) {
...
struct plan {
 ...
 public:
	...
	plan(plan&& other) : h_{std::exchange(other.h_, {})},
  ...
  // TODO(correaa) added one element to avoid problem with gcc 13 static analysis (out-of-bounds)
		...
		while(first_howmany_ < D - 1) {
			...
			// }
			for(int i = first_howmany_ + 1; i != D; ++i) {
				nelems *= which_iodims_[i].second.n;
			}
			if(
 ...
 public:
	...
	void execute(IPtr idata, OPtr odata, int direction) {  // TODO(correaa) make const
		...
		if(first_howmany_ == DD - 2) {
			...
			if(idata == odata) {
				throw std::runtime_error{"complicated inplace 2"};
			}
			for(int i = 0; i != which_iodims_[first_howmany_].second.n; ++i) {
...
struct cached_plan {
	...
	cached_plan(std::array<bool, D> which, boost::multi::layout_t<D, boost::multi::size_type> in, boost::multi::layout_t<D, boost::multi::size_type> out, Alloc const& alloc = {}) {
		...
		if(it == LEAKY_cache.end()) {
			it = LEAKY_cache.insert(std::make_pair(std::make_tuple(which, in, out), plan<D, Alloc>(which, in, out, alloc))).first;
		}
	}
	template<class IPtr, class OPtr>
...
auto dft(std::array<bool, D> which, In const& i, int sign) -> std::decay_t<decltype(dft(which, i, typename In::decay_type(extensions(i), get_allocator(i)), sign))> { return dft(which, i, typename In::decay_type(extensions(i), get_allocator(i)), sign); }
