...
$CXX $0 - o $0x - lcudart - lboost_unit_test_framework - lboost_timer - ldl&& $0x&& rm $0x;
exit
#endif
...
#include <numeric>
	...
	template<class T>
	__attribute__((always_inline)) inline void DoNotOptimize(T const& value) {
...
BOOST_AUTO_TEST_CASE(multi_adaptors_cuda_copy_2d) {
	multi::array<double, 2> A({4, 6});
	std::iota(A.data_elements(), A.data_elements() + A.num_elements(), 1.);
