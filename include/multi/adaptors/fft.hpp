...
namespace fft{
	...
	template<class In, class... Args> auto dft(std::array<bool, std::decay_t<In>::dimensionality> which, In&& in, Args&&... args) -> decltype(auto) {return dft_backward_aux_(priority<1>{}, which, std::forward<In>(in), std::forward<Args>(args)...);}

	template<class... Args> auto dft_forward_aux_(priority<0>, Args&&... args) JUSTRETURN(  fftw::dft_forward(std::forward<Args>(args)...))
	template<class... Args> auto dft_forward_aux_(priority<1>, Args&&... args) -> decltype(auto) {cufft ::dft_forward(std::forward<Args>(args)...);}
	//template<class... Args> auto dft_forward(Args&&... args) DECLRETURN(dft_forward_aux_(priority<1>{}, std::forward<Args>(args)...))
	template<class In, class... Args> auto dft_forward(std::array<bool, std::size_t{std::decay_t<In>::dimensionality}> which, In&& in, Args&&... args) JUSTRETURN(dft_forward_aux_(priority<1>{}, which, std::forward<In>(in), std::forward<Args>(args)...))
	...
	template<class In, class... Args> auto dft_backward(std::array<bool, std::decay_t<In>::dimensionality> which, In&& in, Args&&... args) -> decltype(auto) {return dft_backward_aux_(priority<1>{}, which, std::forward<In>(in), std::forward<Args>(args)...);}
