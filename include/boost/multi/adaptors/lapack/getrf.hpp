...
auto getrf(Context&& ctxt, A&& arr, IPIV&& ipiv){
	assert( ipiv.size() == std::min(arr.size(), (~arr).size()) );
	assert( arr.stride() == 1 );
...
void getrs(Context&& ctxt, LU const& lu, IPIV const& ipiv, B&& barr){
	...
	std::forward<Context>(ctxt).getrs('N', lu.size(), (~barr).size(), lu.base(), (~lu).stride(), ipiv.data(), std::forward<B>(barr).base(), (~barr).stride());
}
...
void getrs_one(Context&& ctxt, LU const& lu, IPIV const& ipiv, V&& barr){
	...
	assert( lu.stride() == 1 );
	assert( barr.stride() == 1 );
	std::forward<Context>(ctxt).getrs('N', lu.size(), 1, lu.base(), (~lu).stride(), ipiv.data(), std::forward<V>(barr).base(), lu.size());
}
