...
\
	...
	if(std::norm(*beta) > std::norm(static_cast<BETA>(0.0))) { BOOST_MULTI_ASSERT1((is_assignable<CC&, decltype((std::declval<ALPHA>()*std::declval<AA>()*std::declval<BB>()) + (std::declval<BETA>()*std::declval<CC>()))> {})); }                                                          \
