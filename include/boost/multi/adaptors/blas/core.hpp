...
template<class XP, class X = typename std::pointer_traits<XP>::element_type, class YP, class Y = typename std::pointer_traits<YP>::element_type, class RP, class R = typename std::pointer_traits<RP>::element_type, enable_if_t<is_c<X>{} && is_c<Y>{} && is_assignable<R&, decltype(/*0.0F+*/ X{}*Y{}+X{}*Y{})>{}, int> =0> void dotu(ssize_t n, XP x, ptrdiff_t incx, YP y, ptrdiff_t incy, RP r) {auto const rr = BLAS(cdotu)(                                      n, reinterpret_cast<c const*>(static_cast<X*>(x)), incx, reinterpret_cast<c const*>(static_cast<Y*>(y)), incy); std::memcpy(reinterpret_cast<std::array<float , 2>*>(static_cast<R*>(r))->data(), &rr, sizeof(rr)); static_assert(sizeof(rr)==sizeof(*r));}  // NOSONAR
...
// NOLINTBEGIN(modernize-use-constraints) TODO(correaa) for C++20
template<class XP, class X = typename std::pointer_traits<XP>::element_type, class RP, class R = typename std::pointer_traits<RP>::element_type, enable_if_t<is_s<X>{} && is_s<R>{} && std::is_assignable<R&, decltype(X{})>{}           , int> =0> void asum(ssize_t n, XP x, ptrdiff_t incx, RP r) {auto rr = BLAS(sasum) (n, reinterpret_cast<s const*>(static_cast<X*>(x)), incx); std::memcpy(reinterpret_cast<s*>(static_cast<R*>(r)), &rr, sizeof(s));} // NOLINT(google-readability-casting,readability-identifier-length)
template<class XP, class X = typename std::pointer_traits<XP>::element_type, class RP, class R = typename std::pointer_traits<RP>::element_type, enable_if_t<is_d<X>{} && is_d<R>{} && std::is_assignable<R&, decltype(X{})>{}           , int> =0> void asum(ssize_t n, XP x, ptrdiff_t incx, RP r) {auto rr = BLAS(dasum) (n, reinterpret_cast<d const*>(static_cast<X*>(x)), incx); std::memcpy(reinterpret_cast<s*>(static_cast<R*>(r)), &rr, sizeof(d));} // NOLINT(google-readability-casting,readability-identifier-length)

template<class XP, class X = typename std::pointer_traits<XP>::element_type, class RP, class R = typename std::pointer_traits<RP>::element_type, enable_if_t<is_c<X>{} && is_s<R>{} && std::is_assignable<R&, decltype(norm(X{}))>{}, int> =0> void asum(ssize_t n, XP x, ptrdiff_t incx, RP r) {auto rr   = BLAS(scasum)(n, reinterpret_cast<c const*>(static_cast<X*>(x)), incx); std::memcpy(reinterpret_cast<s*>(static_cast<R*>(r)), &rr, sizeof(s));} // NOLINT(google-readability-casting,readability-identifier-length)
template<class XP, class X = typename std::pointer_traits<XP>::element_type, class RP, class R = typename std::pointer_traits<RP>::element_type, enable_if_t<is_z<X>{} && is_d<R>{} && std::is_assignable<R&, decltype(norm(X{}))>{}, int> =0> void asum(ssize_t n, XP x, ptrdiff_t incx, RP r) {auto rr = BLAS(dzasum)(n, reinterpret_cast<z const*>(static_cast<X*>(x)), incx); std::memcpy(reinterpret_cast<s*>(static_cast<R*>(r)), &rr, sizeof(d));} // NOLINT(google-readability-casting,readability-identifier-length)

// TODO(correaa) implement workaround for bug in Apple Accelerate BLAS ? https://stackoverflow.com/a/77017238/225186
template<class XP, class X = typename std::pointer_traits<XP>::element_type, class RP, class R = typename std::pointer_traits<RP>::element_type, enable_if_t<is_s<X>{} && is_s<R>{} && std::is_assignable<R&, decltype(X{})>{}           , int> =0> void nrm2(ssize_t n, XP x, ptrdiff_t incx, RP r) {auto rr = BLAS(snrm2) (n, reinterpret_cast<s const*>(static_cast<X*>(x)), incx); std::memcpy(reinterpret_cast<s*>(static_cast<R*>(r)), &rr, sizeof(s));} // NOLINT(google-readability-casting,readability-identifier-length)
template<class XP, class X = typename std::pointer_traits<XP>::element_type, class RP, class R = typename std::pointer_traits<RP>::element_type, enable_if_t<is_d<X>{} && is_d<R>{} && std::is_assignable<R&, decltype(X{})>{}           , int> =0> void nrm2(ssize_t n, XP x, ptrdiff_t incx, RP r) {auto rr = BLAS(dnrm2) (n, reinterpret_cast<d const*>(static_cast<X*>(x)), incx); std::memcpy(reinterpret_cast<s*>(static_cast<R*>(r)), &rr, sizeof(d));} // NOLINT(google-readability-casting,readability-identifier-length)

template<class XP, class X = typename std::pointer_traits<XP>::element_type, class RP, class R = typename std::pointer_traits<RP>::element_type, enable_if_t<is_c<X>{} && is_s<R>{} && std::is_assignable<R&, decltype(norm(X{}))>{}, int> =0> void nrm2(ssize_t n, XP x, ptrdiff_t incx, RP r) {auto rr = BLAS(scnrm2)(n, reinterpret_cast<c const*>(static_cast<X*>(x)), incx); std::memcpy(reinterpret_cast<s*>(static_cast<R*>(r)), &rr, sizeof(s));} // NOLINT(google-readability-casting,readability-identifier-length)
template<class XP, class X = typename std::pointer_traits<XP>::element_type, class RP, class R = typename std::pointer_traits<RP>::element_type, enable_if_t<is_z<X>{} && is_d<R>{} && std::is_assignable<R&, decltype(norm(X{}))>{}, int> =0> void nrm2(ssize_t n, XP x, ptrdiff_t incx, RP r) {auto rr = BLAS(dznrm2)(n, reinterpret_cast<z const*>(static_cast<X*>(x)), incx); std::memcpy(reinterpret_cast<s*>(static_cast<R*>(r)), &rr, sizeof(d));} // NOLINT(google-readability-casting,readability-identifier-length)
// NOLINTEND(modernize-use-constraints) TODO(correaa) for C++20
...
v gemm(char transA, char transB, ssize_t m, ssize_t n, ssize_t k, ALPHA const* alpha, AAP aa, ssize_t lda, BBP bb, ssize_t ldb, BETA const* beta, CCP cc, ssize_t ldc) {  /*NOLINT(bugprone-easily-swappable-parameters)*/              \
	...
	if(!( ldc >= max(sszie_t{1}, m) )) {throw std::logic_error("failed 'ldc >= max(1L, m)' with ldc = "+ std::to_string(ldc) +" and m = "+ std::to_string(m));}                                                                               \
	...
	BLAS(T##gemm)(transA, transB, BC(m), BC(n), BC(k), *reinterpret_cast<T const*>(alpha), reinterpret_cast<T const*>(static_cast<AA*>(aa)), BC(lda), reinterpret_cast<T const*>(static_cast<BB*>(bb)), BC(ldb), *reinterpret_cast<T const*>(beta), reinterpret_cast<T*>(static_cast<CC*>(cc)) /*NOLINT(cppcoreguidelines-pro-type-reinterpret-cast)*/ /*TODO(correaa) check constness*/, BC(ldc)); \
}                                                                                                                                                                                                                        \
