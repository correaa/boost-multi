...
v gemm(char transA, char transB, ssize_t m, ssize_t n, ssize_t k, ALPHA const* alpha, AAP aa, ssize_t lda, BBP bb, ssize_t ldb, BETA const* beta, CCP cc, ssize_t ldc) {  /*NOLINT(bugprone-easily-swappable-parameters)*/              \
	using std::max;                                                                                                                                                                                                                     \
	if(transA == 'N') {BOOST_MULTI_ASSERT1(lda >= max(ssize_t{1}, m));}                                                                                                                                                                               \
	if(transA != 'N') {BOOST_MULTI_ASSERT1(lda >= max(ssize_t{1}, k));}                                                                                                                                                                               \
	if(transB == 'N') {BOOST_MULTI_ASSERT1(ldb >= max(ssize_t{1}, k));}                                                                                                                                                                               \
	if(transB != 'N') {BOOST_MULTI_ASSERT1(ldb >= max(ssize_t{1}, n));}                                                                                                                                                                               \
	BOOST_MULTI_ASSERT1( aa != cc );                                                                                                                                                                                                          \
	BOOST_MULTI_ASSERT1( bb != cc );                                                                                                                                                                                                          \
	if(!( ldc >= max(ssize_t{1}, m) )) {throw std::logic_error("failed 'ldc >= max(1L, m)' with ldc = "+ std::to_string(ldc) +" and m = "+ std::to_string(m));}                                                                               \
