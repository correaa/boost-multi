...
auto gemv_n(Context ctxt, typename MIt::element a, MIt m_first, Size count, XIt x_first, typename MIt::element b, YIt y_first) {  // NOLINT(readability-identifier-length) BLAS naming
	...
	if constexpr(! is_conjugated<MIt>::value) {
		...
		else if((*m_first).size()==1) {ctxt->gemv('T', (*m_first).size(), count, &a, m_first.base()            , m_first. stride(), x_first.base(), x_first.stride(), &b, y_first.base(), y_first.stride());}
		else                          {throw gemv_stride_error{"not BLAS-implemented L29"};}  // LCOV_EXCL_LINE
	} else {
		...
		else                          {throw gemv_stride_error{"not BLAS-implemented L32"};}  // LCOV_EXCL_LINE
	}
