...
The FFT Adaptor provides an interface for the FFT libraries, in particular FFTW (in CPU) and cuFFT/hipFFT (in GPU) libraries.
The library allows to abstract the low level aspect of these libraries, and it also provides a functional interface.
...
Value types can be of arbitrary dimension (`multi::array<complex, D>`).
Views can be assigned to subarrays (e.g. `multi::subarray<complex, D>`.
Element types are contrained to `std::complex<double>` or binary compaible (e.g. `thrust::complex<float>`).

In this interface, functions generates views that can be assigned to values or from which constructors can be called in the library without unnecessary copies or allocations.
Expressions such as `multi::fft::dft(A)`, produce a range object that can be used in larger expressions, such as construction and assignemnt.

- Construction:

```cpp
multi::array<complex, D> const B = multi::fft::dft_forward(A);
```
