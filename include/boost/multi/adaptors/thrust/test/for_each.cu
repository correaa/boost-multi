...
class auto_timer : std::chrono::high_resolution_clock {
 ...
 public:
	...
	// ~auto_timer() {
	// 	cudaDeviceSynchronize() == cudaSuccess ? void() : assert(0);
	// 	auto const count = std::chrono::duration<double>(now() -
	// start_).count(); 	std::cerr << label_ << ": " << count << " sec\n";
	// }
};
}  // namespace
...
auto main()
	...
	{
		auto cpu     = multi::array<int, 2>({64, 1024}, 0);
		auto cpu_par = multi::array<int, 2>({64, 1024});
