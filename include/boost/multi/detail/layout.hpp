...
struct bilayout {
 ...
 public:
	bilayout(bistride_type bistride, sub_type sub, Ptr origin)
	...
	auto stride() const {
		class stride_t {
		 ...
		 public:
			explicit stride_t(std::ptrdiff_t vv, Ptr origin)
			: vv_{vv}
			, origin_{origin} {}
			auto operator*(std::ptrdiff_t nn) const { return stride_t{nn * vv_, origin_}; }
