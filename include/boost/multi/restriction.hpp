...
template<dimensionality_type D, class Proj, class Ret>
class restriction_elements_iterator : ra_iterable<restriction_elements_iterator<D, Proj, Ret>> {
 ...
 public:
	...
	using value_type        = Ret;             // decltype(apply_(std::declval<Proj&&>(), std::declval<typename extensions_t<D>::elements_t::iterator::value_type>()));  // (std::declval<typename std::decay_t<A>::element>(), std::declval<typename std::decay_t<As>::element>()...));  // difference_type;
	using pointer           = void;
	using reference         = Ret;  // decltype(apply_(std::declval<Proj&&>(), std::declval<typename extensions_t<D>::elements_t::iterator::value_type>()));
	using iterator_category = std::random_access_iterator_tag;
...
class restriction_elements_t {
 ...
 public:
	...
	BOOST_MULTI_HD constexpr auto operator[](index idx) const -> decltype(auto) {
		// using std::apply;
		return apply_(proj_, elems_[idx]);
	}
	...
	using iterator = restriction_elements_iterator<D, Proj, reference>;
