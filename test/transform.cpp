...
//static constexpr struct neg_t{
//	template<class T>
//	constexpr auto operator()(T const& x) const{return -x;}
//} neg;
MAYBE_UNUSED static auto const neg = [](auto const& x){return -x;};
...
public:
	...
	using value_type = typename std::iterator_traits<It>::value_type;
	using pointer  = involuter<typename std::iterator_traits<It>::pointer, F>; //void;
	using iterator_category = typename std::iterator_traits<It>::iterator_category;

	template<class Other, decltype(boost::multi::implicit_cast<It>(std::declval<Other const&>().it_))* =nullptr>
	         constexpr involuter(Other const& other) : it_{other.it_}, f_{other.f_}{}
	template<class Other, decltype(boost::multi::explicit_cast<It>(std::declval<Other const&>().it_))* =nullptr>
	explicit constexpr involuter(Other const& other) : it_(other.it_), f_{other.f_}{}

	explicit constexpr involuter(It it) : involuter(
		std::move(it), 
		default_construct<F>()
//		[&]{if constexpr(sizeof(F)<=1) return *(F*)(this); else return F{};}()
	...
	constexpr auto operator*() const{return reference{*it_, f_};}
	constexpr involuter& operator+=(difference_type n){it_+=n; return *this;}
	constexpr involuter operator+(difference_type n) const{return involuter(*this)+=n;}
	constexpr pointer operator->() const{return pointer{&*it_, f_};}
};
...
static_assert( std::is_convertible< negater<double*>, negater<double const*> >{}, "!" );
...
BOOST_AUTO_TEST_CASE(negater){
	{
		double d = 1.;
		test::negater<double*> p(&d);
		test::negater<double const*> pc = p;
		BOOST_REQUIRE( *pc == -d );
	}
}
