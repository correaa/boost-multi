...
* `MultiArrayView::(const_)iterator`:
...
* `MultiArrayView::(const_)reference`: 
Reference to subarrays of lower dimension. For `D > 1` this are not true language-references but types that emulate them (with reference semantics).
For `D == 1` this is a true language reference to an element type (`T&`).
