...
The code is developed for several compilers with standard C++17 support: for example LLVM's `clang` (9+), GNU's `g++` (7+), Nvidia's `nvcc` (11.3+), `nvc++` (20.7-21.3+) and Intel's `icpc` (2021.2.0+), `icpx` (2022.0.2+).
...
* `multi::array<T, D, A>`: Array of dimension `D`, it has value semantics if `T` has value semantics. Memory is requested by allocator of type `A`, should support stateful and polymorphic allocators.
* `multi::array_ref<T, D, P = T*>`: Array interpretation of a random access range, usually a contiguous memory block. It has reference semantics. Thanks to (non-virtual) inheritance an `array<T, D, A>` is-a `array_ref<T, D, A::pointer>`.
