...
\begin{alertblock}{Key differences}
  ...
  Multi subarrays are \textbf{always views}; a copy requires an explicit owning construction.
...
\begin{frame}[fragile,label=alg1]{Algorithms (raw loops)}
...
\begin{frame}[fragile,label=alg2]{Algorithms (algorithm seasoning)}
  Take a 64x64x64 array (\(\sim 250\mathrm{k}\) elements) \lstinline|A|, apply a compute-bound op.
  \begin{lstlisting}
...
\begin{frame}[fragile,label=alg3]{Algorithms (algorithm parallelization)}
  Take a 64x64x64 array (\(\sim 250\mathrm{k}\) elements) \lstinline|A|, apply a compute-bound op.
  \begin{lstlisting}
...
\begin{frame}[fragile,label=alg4]{Algorithms (attempt to parallelize in the GPU)}
  Take a 64x64x64 array (\(\sim 250\mathrm{k}\) elements) \lstinline|A|, apply a compute-bound op.
  \begin{lstlisting}
...
\begin{frame}[fragile,label=alg5]{Algorithms (optimized)}
  Take a 64x64x64 array (\(\sim 250\mathrm{k}\) elements) \lstinline|A|, apply a compute-bound op.
  \begin{lstlisting}
...
\begin{frame}[fragile,label=alg6]{Algorithms (optimized)}
  Take a 64x64x64 array (\(\sim 250\mathrm{k}\) elements) \lstinline|A|, apply a compute-bound op.
  \begin{lstlisting}
...
\begin{frame}[fragile,label=alg-summary]{Summary}
...
\begin{frame}[fragile,label=gpurun1]{\texttt{gpu::run} Triple Loop: Potential on a 3D Grid}
%// hamiltonian/self_consistency.hpp:208
\begin{lstlisting}
// Add uniform electric field contribution to the scalar potential:
//   V(r) += -E . r   (over all real-space grid points)
auto efield = pert_.uniform_electric_field(time);
gpu::run(vscalar.basis().local_sizes()[2],   // iz: slowest index
         vscalar.basis().local_sizes()[1],   // iy
         vscalar.basis().local_sizes()[0],   // ix: fastest index
    [point_op = vscalar.basis().point_op(),
     efield,
     _vscalar = vscalar.cubic().begin()]
...
\end{lstlisting}
\begin{itemize}
  \item Dimensions are passed outermost-first; indices arrive innermost-first
  \item On GPU: each thread handles one $(i_x, i_y, i_z)$ triplet
  \item On CPU: equivalent to three nested \texttt{for} loops
  \item \texttt{vscalar.cubic()} returns a 3D \texttt{gpu::array} view ---
        indexing is \texttt{[ix][iy][iz]} as expected
\end{itemize}
\tiny\texttt{src/hamiltonian/self\_consistency.hpp:208}
\end{frame}

\begin{frame}[fragile,label=gpurun2]{\texttt{gpu::run} Reduction: Occupation-Weighted Energy Sum}
\begin{lstlisting}
// Mixed loop + reduction: loop over states, reduce over basis points
auto lred = gpu::run(nstates, gpu::reduce(nbasis), 0.0,
    GPU_LAMBDA (auto ist, auto ip) -> double {
        return /* contribution from state ist, point ip */;
    });
\end{lstlisting}
\begin{itemize}
  \item \texttt{gpu::reduce(N)} marks a dimension for summation
  \item The lambda must return the \textbf{per-element contribution}
  \item Loop and reduce dimensions can be mixed freely
\end{itemize}
\tiny\texttt{src/hamiltonian/energy.hpp:45} \quad
\texttt{external\_libs/gpurun/include/gpu/reduce.hpp:65}
\end{frame}
...
\begin{frame}[label=inq0]{INQ, electronic structure code for HPC systems}
	\centering
...
\begin{frame}[fragile,label=inq1]{\texttt{gpu::array}: Portable CPU/GPU Arrays via a Type Alias}
\begin{lstlisting}
...
\begin{frame}[fragile,label=inq2]{Custom-Typed 1D Arrays for Atomic Properties}
\begin{lstlisting}
...
\begin{frame}[fragile,label=inq3]{Dynamic Resizing: k-points $\times$ Eigenstates}
\begin{lstlisting}
...
\begin{frame}[fragile,label=inq4]{4D Fields: Reinterpretation and Axis Rotation}
...
\begin{frame}[fragile,label=inq5,shrink]{FFTW / cuFFT via a Unified Multi Adaptor}
\begin{lstlisting}
