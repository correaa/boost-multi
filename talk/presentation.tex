...
\begin{frame}{A bit about myself}
  \begin{itemize}
    ...
    \item Simulating Quantum Mechanics using Classical Computers, HPC, and GPUs \\
      ~
    \item LLNL Representative at the C++ Standards Committee \\
      ~
    % \item \textsc{Multi} Multi-dimensional arrays in C++
    % \item \textsc{B-MPI3} C++ wrapper for the MPI 3.0 standard (MP, RMA, ShMem) \\
    %   ~
    \item \textsc{Inq} Electronic structure code for ground state and time dependent
...
\begin{itemize}
  ...
  \item Many false starts with C++: object-oriented design, pointless template metaprogramming, multiple dispatching, DSLs
...
\begin{frame}{Why Arrays?}
  \begin{columns}
    \begin{column}{0.5\textwidth}
      ...
      \begin{itemize}
        \item Contiguous memory $\Rightarrow$ cache efficiency
...
\begin{tikzpicture}[scale=0.78, every node/.style={font=\small}]
  ...
  % --- 2D ---
  \node[anchor=east] at (4.0, 2.15) {\textbf{2D}};
  % row 0
...
\begin{frame}[fragile]{Claim: 	(Semi)Regular types are amenable to equational reasoning.}
	...
	\begin{alertblock}{Litmus test for your type \lstinline{T}}
		Quick test for your types, does this compile? does it do the expected thing?
		\begin{lstlisting}
std::vector<T> v = {r1, r2, r3};
...
\small
\begin{tabular}{llll}
\textbf{Type} & \textbf{Owns?} & \textbf{Mutable elements?} & \textbf{Returned by} \\
\hline
\lstinline|multi::array<T, D>|          & Yes & Yes & you create it \\
\lstinline|multi::array_ref<T, D>|      & No  & Yes & wrapping raw pointer \\
\lstinline|multi::subarray<T, D>|       & No  & Yes & \lstinline|A[i]|, \lstinline|A.sliced()|, ... \\
\lstinline|multi::const_subarray<T, D>| & No  & No  & from \lstinline|const| array \\
\lstinline|multi::dynamic_array<T, D>|  & Yes & Yes & from \lstinline|const| array \\
\end{tabular}
\end{table}
