...
\begin{frame}{\textsc{Multi} was proposed to Boost (2016) and conditionally accepted}
\centering
\url{gitlab.com/correaa/boost-multi}\\
\includegraphics[scale=0.1]{gitlab}\\
\url{github.com/correaa/boost-multi}\\
...
and element references all carry the allocator's pointer.
...
\end{columns}
\begin{lstlisting}
template<typename T, dimenensionality_type D, class Alloc>
class array {
...
\begin{itemize}
  \item When possible same code should work for different backends
  \item Functional style of programming (deviates from \texttt{std::linalg})
...
multi::array<double, 2, alloc> A({3, 3});
multi::array<double, 2, alloc> B({3, 3});
multi::array<double, 2, alloc> C({3, 3});
\end{lstlisting}
\end{column}
\end{columns}
\begin{lstlisting}
C = blas::gemm(1.0, A, B);  // C = A*B, shouldn't allocate
\end{lstlisting}
\begin{center}
\includegraphics{dispatching_blas}
\end{center}
\end{frame}
...
\begin{enumerate}
  \item \textbf{Generic programming works for arrays. (if you make some concession)}\\
