...
Pros:
\begin{itemize}
  \item Maintains as a much as possible symmetry between dimensions
  \item Memory of arrays and subarrays is compact, regular, an often contiguous
  \item Compatible with legacy libraries, FFTW, BLAS, Lapack, CUFFT, CUBLAS, MPI
\end{itemize}
Cons:
\begin{itemize}
  \item Not available in the language
  \item Not support from STL algorithms
  \item Not easy to grow
\end{itemize}

\begin{alertblock}{Other datastructures (layouts) also maintain symmetry}
  Tiled, Zig-zag, Hilbert curve filling
\end{alertblock}

\end{frame}

\begin{frame}{Multidimensionality via nested contatainers (the wrong way)}
\begin{columns}[T]
\begin{column}{0.5\textwidth}
...
\begin{itemize}
  ...
  \item Short time horizon (I am stuck in C++17/20 for compatibility with GPUs)
\end{itemize}
...
double data[12] = {};
multi::array_ref<double, 2> R({3, 4}, data);

// element access
A[1][2] = 3.14;
A(1, 2) = 3.14;  // equivalent
A[1, 2] = 3.14;  // equivalent C++26
\end{lstlisting}
...
P0672R0 (2017) by Joël Falcou, Herb Sutter et al.
(\texttt{std::vector<bool>}, proxy objects)
\end{alertblock}

My recommendation \emoji{hot-pepper}: use the provided `auto-plus operator' \texttt{auto ... = + ...;}
\begin{lstlisting}
\end{lstlisting}
...
\begin{itemize}
  \item Use provied `auto-plus' operator
...
What to avoid bloat? (or work across compilation boundaries?)
\begin{lstlisting}
...
\textbf{Type}                              & \textbf{Owns} & \textbf{Copy} & \textbf{Move} & \textbf{Mutable elems?} & \textbf{Returned by}                          \\
\hline
