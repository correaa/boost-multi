...
\begin{alertblock}{Visualization is only a convention}
  Desired symmetry between rotated (transposed) illustrations of the arrays.
...
Pros:
\begin{itemize}
  \item Maintains as much as possible symmetry between dimensions
...
Some types might not have equality in practice because it could be prohibitively expensive or the algorithm for equality is not known.
...
// from extents
multi::array<int, 2> A({3, 4});  // a 3 x 4
...
\begin{alertblock}{Inheritance for composition (usually a bad idea)}
  Made it \texttt{public} inheritance \emoji{hot-pepper} because I found it useful.
  It is opt-in.
\end{alertblock}
\end{frame}

\begin{frame}[fragile]{\texttt{multi::dynamic\_array}: pinned owning array}
...
\item Slice\\
  \lstinline|A.sliced(first, last)| range subarrays (equal dimension) semiopen interval \([\mathrm{first}, \mathrm{last}) \)
\end{itemize}
...
\begin{itemize}
  \item Indexing (with parenthesis)
    \lstinline|A(i) -> A[i]|
  \item Sliced (terse)
...
\centering
\includegraphics[scale=0.8]{types_relations}

\begin{alertblock}{}
  Looks complicated, intuitive for the most part in practice.
\end{alertblock}
\end{frame}
...
// error! subarray<int, 2> is no copyable
\end{lstlisting}
...
\setbeamercolor{block body alerted}{bg=green!10}
\begin{alertblock}{Passes!}
  Indicates that \lstinline|swap|, \lstinline|rotate|, \lstinline|stable_partition|
\end{alertblock}
}
\end{frame}
...
Translated to C++, \(f\) could be function object that maps coordinates to values, and
...
\begin{frame}[fragile]{Generic Serialization}

Arrays integrate with \textbf{Boost.Serialization} and \textbf{Cereal} out of the box.
...
std::fstream ofs{"data.xml"};
boost::archive::xml_oarchive{ofs} << boost::nvp("A", A);

multi::array<double, 2> B;
std::ifstream ifs{"data.xml"};
boost::archive::xml_iarchive{ifs} >> B;
assert(A == B);
\end{lstlisting}
