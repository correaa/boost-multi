...
c++ - std = c++ 17 `# - DNDEBUG` - Ofast - Wall - Wextra $0 - o $0x.x - lpthread && $0x.x 16384 128 128 $ @ && rm - f $0x.x;
exit
#endif
...
auto tr_prod_block2(Matrix1 const& A, Matrix2 const& B) {
	...
	if(A.size() > 1024) {
		...
		return +tr_prod_block2(A({0, i2}, {0, j2}), B({0, j2}, {0, i2})) + tr_prod_block2(A({0, i2}, {j2, bs}), B({j2, bs}, {0, i2})) + tr_prod_block2(A({i2, as}, {0, j2}), B({0, j2}, {i2, as})) + tr_prod_block2(A({i2, as}, {j2, bs}), B({j2, bs}, {i2, as}));
	}
}
...
int main() {
	...
	cout << tr_square_block2(A) << '\n';
	{
		...
		double warm = tr_square_direct(A);
		cout << warm << '\n';
		std::iota(A.data_elements(), A.data_elements() + N * N, 1.23);
		multi::array<double, 2> B(A.extensions());
		B[10][10] = 1.;
		double t1 = getTime();
