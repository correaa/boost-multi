...
clang++ - O3 - Ofast - std = c++ 14 - DNDEBUG - Wall - Wextra - Wpedantic - Werror $0 - o $0.x - lboost_timer && time $0.x $ @ && rm - f $0.x;
exit
#endif
...
int main() {
	...
	assert(0);  // check that NDEBUG is on
	{
		...
		iota(begin(data), end(data), 1.2);
		iota(begin(data), end(data), 10.1);
		data[1234] = 929.1;
		double sum_raw;
		{
			double                       sum = 0.;
		...
		iota(begin(data), end(data), 1.2);
		iota(begin(data), end(data), 202.2);
		data[1234] = 399.1;
		double sum_2D;
		{
			double                       sum = 0.;
		...
		iota(begin(data), end(data), 1.2);
		iota(begin(data), end(data), 10.112);
		data[1234] = 99.1;
		double sum_raw2;
		{
			double                       sum = 0.;
		...
		iota(begin(data), end(data), 1.2);
		iota(begin(data), end(data), 33.112);
		data[1234] = 1199.1;
		double sum_raw_acc;
		{
