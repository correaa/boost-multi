...
remove_all(dir);
{
	...
	// marray<int     , 1> arr1(marray<int     , 1>::extensions_type({0, 10}), m.get_allocator());

	auto&& arr1d = *m.construct<marray<int     , 1>>("arr1d")(10        , m.get_allocator());
	auto&& arr2d = *m.construct<marray<double  , 2>>("arr2d")(std::make_tuple(1000, 1000), 1.0, m.get_allocator());
//  auto&& arr3d =
//      *m.construct<marray<unsigned, 3>>("arr3d")(multi::extensions_t<3>{10, 10, 10}, 1u , m.get_allocator());
//  auto&& arr3d_cpy =
//      *m.construct<marray<unsigned, 3>>("arr3d_cpy")(multi::extensions_t<3>{0, 0, 0}, m.get_allocator());
