...
->decltype(::core::syev('V', uplo==blas::filling::upper?'L':'U', size(a), base(a), stride(a), base(w), base(work), size(work), std::declval<int&>()), a({0l, 1l}, {0l, 1l}))
{
	...
	if(stride(rotated(a))==1){
		...
		::core::syev('V', uplo==blas::filling::upper?'U':'L', size(a), base(a), stride(rotated(a)), base(w), base(work), size(work), info);
...
decltype(auto) syev(blas::filling uplo, Array2D&& a, Array1D&& w){
	return syev(uplo, a, w, Array1DW(std::max(1l, 3*size(a)-1l), get_allocator(w))); // TODO obtain automatic size from lapack info routine
}
...
auto syev(blas::filling uplo, Array2D const& a){
	struct{
		...
		multi::array<typename Array2D::element_type, 1, decltype(get_allocator(a))> eigenvalues;
	} ret{
		a.decay(), 
		multi::array<typename Array2D::element_type, 1, decltype(get_allocator(a))>(size(a), get_allocator(a))
	};
