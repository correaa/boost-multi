...
class dot_ptr{
	...
	ItY  y_first_;
protected:
	dot_ptr(Context&& ctxt, ItX x_first, Size count, ItY y_first) : ctxt_{std::forward<Context>}, x_first_{x_first}, count_{count}, y_first_{y_first}{}
public:
	...
	->decltype(blas::dot_n(Context{}  , std::declval<ItX>(), Size{}      , std::declval<ItY>(), d_first), d_first + count){assert(count == 1);
...
template<class Context, class X, class Y, class Ptr = dot_ptr<typename X::const_iterator, typename X::size_type, typename Y::const_iterator>>
...
dot_ref<X, Y> dot(Context&& ctxt, X const& x, Y const& y){return {std::forward<Context>(ctxt), x, y};}

template<class X, class Y> [[nodiscard]] 
dot_ref<X, Y> dot(X const& x, Y const& y){return {Context{}, x, y};}
