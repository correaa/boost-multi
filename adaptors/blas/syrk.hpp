...
int main(){
	...
	}
	{
		...
		multi::array<complex, 2> C({2, 2}, 9999.);
		syrk(1., rotated(A), 0., C); // C^T = C =  A*A^T = (A*A^T)^T, A*A^T, C are C-ordering, information in C lower triangular
		print(C) <<"---\n";
	}
	{
		multi::array<complex, 2> const A = {
		...
		multi::array<complex, 2> C({2, 2}, 9999.);
		syrk(1., rotated(A), 0., rotated(C)); // C^T = C =  A*A^T = (A*A^T)^T, A*A^T, C are C-ordering, information in C upper triangular
		print(C) <<"---\n";
	}
	{
		multi::array<complex, 2> const A = {
