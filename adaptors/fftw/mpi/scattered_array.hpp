...
public:
	scattered_array(multi::extensions_type_<2> exts, bmpi3::communicator comm, Alloc alloc = {}) : 
		local_distrubution_{exts, comm},
		alloc_{alloc},
		local_data_{alloc_.allocate(local_distrubution_.local_count())},
		second_ext_{std::get<1>(exts)}
	{}
	array_ref <T, 2, local_pointer_type> local_cutout()      &{return array_ref <T, 2, local_pointer_type>(local_data_, local_distrubution_.local_extension()*second_ext_);}
	array_cref<T, 2, local_pointer_type> local_cutout() const&{return array_cref<T, 2, local_pointer_type>(local_data_, local_distrubution_.local_extension()*second_ext_);}
};
...
int mpi3::main(int, char*[], mpi3::environment& menv){
	...
	mpi::scattered_array<double, 2> A({8, 15}, world);
	
	if(auto x = A.local_cutout().extensions())
		for(auto i : std::get<0>(x))
			for(auto j : std::get<1>(x))
				A.local_cutout()[i][j] = 8000000 + 10000*j + 800 + i;//std::complex<double>(i + j, i + 2*j + 1)/std::abs(std::complex<double>(i + j, i + 2*j + 1));
