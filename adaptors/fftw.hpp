...
struct flags{
	using underlying_type = decltype(FFTW_PRESERVE_INPUT);
private:
...
>
auto fftw_plan_many_dft(It1 first, It1 last, It2 d_first, int sign, unsigned flags)
...
auto fftw_plan_dft(In const& in, Out&& out, int s, unsigned flags){
