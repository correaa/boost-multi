...
template<class T>
       auto has_num_elements_aux(T const& t)->decltype((void)std::declval<T const&>().num_elements(), std::true_type {});
...
//inline auto base(std::tuple<> const& a){return a;}
...
//template<class T, std::size_t N>
//constexpr auto origin(T(&t)[N]) noexcept{return reinterpret_cast<std::remove_all_extents_t<T[N]>*>(&t);} // NOLINT(cppcoreguidelines-avoid-c-arrays,hicpp-avoid-c-arrays,modernize-avoid-c-arrays) : for backwards compatibility
...
template<class T, std::size_t M, std::size_t N>
constexpr auto dimensionality(std::array<std::array<T, M>, N> const& arr) -> dimensionality_type{return 1 + dimensionality(arr[0]);}
