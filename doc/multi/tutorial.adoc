...
Note that since there is no immediate need to assign elements, and the target array can be conveniently declared `const` if no further mutation is necessary).
...
The allocation would only happen at the point at which the result is materialized as a concrete copy.
...
Another possible application of restrictions is the ability to define lazy expressions, also known as expression templates.
...
The lazy operations utilized here can beimplemented in a fairly intuitive way using a series of array restrictions (C++23 for simplicity):
...
Note that, again, none of the intermediate operations generate temporary concrete (allocated) arrays.
...
There is no single way to implement lazy operations; it depends strongly on the intended use and small details can lead to divergent semantics.
...
For lambdas that capture references, the validity of the restriction objects is be contigent to the lifetime of these references.
Analogously if the lambdas capture by copy, the implications of copying must be evaluated too.
