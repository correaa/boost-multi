...
D = A + B + C;  // this operation will not generate an intermediate array and will not allocate (if D has the correct sizes)
```
...
In this example, the `1` is "repeated" or interpreted as `{1, 1, 1}` (without copies) to match the left operand's dimensions.
```cpp
...
Therefore, we should treat expressions as references to arrays.
Expressions do not have independent value semantics.

To use operators, explicitly import them into your current namespace, as shown in the examples; or using `using namespace multi::broadcast` is possible but not recommended.
...
But fundamentally, these operations return expression objects with different semantics, not concrete arrays.
