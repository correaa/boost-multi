...
The library provides a series of operator overloads to generate expression that can be lazily evaluated.
...
D = A + B + C;  // this operation will not allocate (if D has the correct sizes)
```
link:https://godbolt.org/z/neK16a4Yn[(live)]

The key feature of these operations is that they provide automatic "broadcasting", which is convenient for simplifying notation.
