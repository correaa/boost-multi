...
So far, we have examined the use of arrays that contain elements or references to elements stored in memory;
these are referred to as container arrays or concrete arrays.
...
...
```
Note that `R` does not request memory to store the values; it generates them only when needed.
...
They also provide a complete iterator inteface, `.begin()`, `.end()`, `.home()` and they can be used as standard ranges.
...
A key difference with concrete arrays is that their elements cannot be mutated, and that there are no internal pointers or layouts to extract (e.g., through functions like `.data_elements()`, `.base()`, `.layout()` or `.stride()`).

A first use of restrictions is for generating concrete arrays from coordinates, to replace the following commonly used nested loop for initialization:
```cpp
...
Note that target array can be conveniently declared `const` if no further mutation is necessary.
The element initialization here is delegated to the library;
the initialization could occur using optimal hardware- or library-specific operations (e.g., on a GPU device) and in any order.
...
auto concrete_shifted = shift(concrete);  // doesn't allocate (hodls a reference to concrete)
```
...
The allocation would only happen (if at all) at the point at which the result is materialized as a concrete copy.
...
The library provides a facility and a series of operator overloads to generate expression that are lazily evaluated.
Lazy expressions are useful to generate complex array equations that are evaluated in element-wise fashion without the involvement of temporary arrays or storage.
These operations include summation, multiplication (elementwise), together with most special functions (`exp`, `abs`, `sin`, `cos`, ...)

```cpp
#include <multi/broadcast.hpp>  // brings operators
...
...
D = A + B + C;  // this operation will not generater an intermediate array and will not allocate (if D has the correct sizes)
```
link:https://godbolt.org/z/neK16a4Yn[(live)]

The key feature of these operations is that they provide automatic "broadcasting", which is convenient for simplifying notation, and it is common in modern array languages.
Broadcasting (as it is known in Numpy and in Matlab), or scalar extensions (APL) is a feature that allows to reinterpret binary operations with mismatching dimensionality.
Generally dimensionality mismatch are handled by broadcasting, but size mismatch are not accepted (assertions try to catch these in debug compilation mode.)

In this case, the scalar is "repeated" effectively to `{1, 1, 1}` (without copies) to match the dimensionality of left operand.
```cpp
...
Similarly a scalar (a non-array) can be also promoted into higher dimensionality if necessary:
```cpp
multi::array<int, 2> const A = {
	{1, 2, 3},
	{4, 5, 6}
...
The same behavior applies to non-scalars, to either side of the operation:
...
Expressions can be named (like above `expr`), but do not own the objects utilized in the expression.
Most elements in the expression are not owned and arrays elements are never copied or allocated.
Therefore we should regard expressions as references to external arrays, they don't have the independent sematics of a value.

Operators need to be explicitly pulled from the broadcast namespace, like in the examples or by `using namespace multi::broadcast` (not recommended).
This is to avoid surprises by confusing broadcast elementwise operations with other common algebraic operations such matrix multiplication or matrix exponentiation;
but fundamentally because the results of these operations are not concrete arrays, but expression objects with a different semantics.

For example, returning expressions that refer tp local array variables from a C++ function generally produces dangling references.

Most expressions should be immediately evaluated into a concrete arrays, idealy in a single line.
A common pattern consists in materializing the expression into an a concrete array, either by construction or by assignment:
...
// or auto expr_copy1 =+ 2*b + A / exp(B);
```

Lazy expressions are fully integrated with the rest of the library, meaning that they can be utilized generically in the same way as immutable concrete arrays.
(Also they don't have a layout or an internal pointer.)
...
Custom functions can be applied by using the `multi::broadcast::map` function:
```
auto const& AVE = multi::broadcast::map([](auto a, auto b) { return (a + b) / 2.0 ; }, A, B);
```
