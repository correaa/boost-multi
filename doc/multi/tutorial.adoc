...
In simple terms, the constructors of such type of elements are specified and do nothing, not even set values.
...
Finally, the library offers a tag to explicitly state the intention of having uninitialized elements: `multi::arrray<int, 2> A2({2, 3}, multi::uninitialized)`.
This syntax will compile only if elements are trivially-default constructible, for example, `multi::arrray<std::string, 2> A2({2, 3}, multi::uninitialized)` will not compile because `std::string` does not have a trivial constructor.
