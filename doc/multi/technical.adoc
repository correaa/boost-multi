...
Since C++23, (or when `__cpp_multitdimensional_subscript` is defined), element access supports multidimensional subscript operators: `A[i, j]` is equivalent to `A[i][j]`, `A[i, j, k]` is equivalent to `A[i][j][k]`, etc. for indices `i` , `j`, `k`, ... .
