...
however, it is convenient for documentation to present the classes in a different order since the classes `subarray`, `array_ref`, `dynamic_array`, and `array` have an *is-a* relationship (from left to right), through C++ public inheritance.
...
A subarray-reference of type `T` and non-negative dimension `D`.
...
|`element_ref`     | Constant reference to element type, deduced from pointer type, typically `T const&`
