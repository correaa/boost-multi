...
|`index_range`     | describe ranges of indices, constructible from braced index types or from an extension_type. Can be contiguous (e.g. {2, 14}) or strided (e.g. {2, 14, /*every*/ 3})
...
Reference-like types have corresponding pointer-like types that provide an extra level of indirection and can be rebound (like language pointers);
...
| `elements`                                  | a flattened random-access view of all the elements rearranged canonically. `A.elements()[0] -> A[0][0]`, `A.elements()[1] -> A[0][1]`, etc. The type of the result is not a subarray but a special kind of random-access range. Takes no arguments.
...
| `elements`        | a flattened random-access and contiguous view of all the elements in the array in canonical order. (This is overwritten from the base class, to provide contiguous access)
|===
...
| (destructor)      | Destructor deallocates memory and destroys the elements
| `operator=`       | assigns the elements from the source, sizes must match.
| `elements`        | A flattened random-access and contiguous view of all the elements in the array in canonical order.
|===
...
| (constructors)    | `array::array({e1, e2, ...}, T val = {}, Alloc = {})` constructs a D-dimensional array by allocating elements; `array::array(It first, It last)` and `array::array(Range const& rng)`, same for a range of subarrays. `array::array(std::initializer_list<...>, Alloc = {})` constructs the array with elements initialized from a nested list.
| (destructor)      | Destructor deallocates memory and destroys the elements
...
| (constructor)    | takes 2 arguments, an extensions type of dimension `D` and a function that takes `D` arguments
...
|`index`           | indexing type in the leading dimension (usually `std::ptrdiff_t`)
|`size_type`       | describe size (number of subarrays) in the leading dimension (signed version of pointer size type, usually `std::ptrdiff_t`)
|`index_range`     | describe ranges of indices, constructible from braced indices types or from an extension_type. Can be contiguous (e.g. {2, 14}) or strided (e.g. {2, 14, /*every*/ 3})
|`extension_type`  | describe a contiguous range of indices, constructible from braced index (e.g. `{0, 10}`) or from a single integer size (e.g. 10, equivalent to {0, 10}).
|`difference_type` | describe index differences in leading dimension (signed version of pointer size type, usually `std::ptrdiff_t`)
...
|`end`             | returns an `iterator` to the end of the sequence in the leading dimension
