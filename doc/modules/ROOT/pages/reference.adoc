...
A restriction is an array to generates its elements on the fly.
It is defined by combining a D-dimensional function with an D-dimensinal extensions.
...
| (constructor)    | takes extensions of dimension `D` and a function that takes `D` arguments
...
A factory function `multi::restricted(F fun, multi::extensions_t<D> exts)` can be used to generate this restriction objects.
`fun` must be a function that returns deterministically (e.g. doesn't depend on the order of evaluation), `F` must be "regular invokable" (`std::regular_invokable<F, Args...>` is `true` and `sizeof...(Args)` the number of dimensions `D`.)
