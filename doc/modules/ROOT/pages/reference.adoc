...
| `multi::array<T, D, ...>`         | Static         | Dynamic/Mutable           | Heap (allocates)                    | Yes, transferable (movable)    | General purpose (value type), limited constexpr
| `multi::array_ref<T, D>`          | Static         | Dynamic/Immutable         | Any  (doesn't allocate)             | No, (pinned)                   | Use existing buffers (e.g. from other libs)
| `multi::inplace_array<T, D, ...>` | Static         | Dynamic/Mutable (bounded) | Stack (doesn't heap-allocate)       | Yes, non-transferable (pinned) | Fast, stack constrained, lowest level of divide and conquer, fully constexpr
