...
For most uses, a `multi::array` should be preferred instead, use `unique_array` only when fine grained restrictions over allocations needs to be enforced at compile time.
...
The destructor deallocates memory and destroys the elements
`operator=` assigns the elements from the source (sizes must match), it also supports move semantic in other cases.
