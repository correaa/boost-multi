...
It is represented by `multi::cursor_t<ElementPtr, D, StridesType>` and obtained via the `.home()` member of any array or subarray, which returns a cursor pointing to the top-corner element at index stem:[[0, 0, \ldots, 0\]].
