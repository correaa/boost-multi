...
The main idea is that the extensions of an array doesn't need to be hard coded, for applications such as simulations the sizes adapt to the problem sizes, or set interactively/input by the user or from a previous run.
This is a useful combination for medium to large arrays,
therefore the library mainly provides arrays which sizes can be determined at run time.
