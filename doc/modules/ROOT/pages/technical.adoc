...
The C++ abstract machine imposes strict rules on pointer arithmetic ([expr.add]):
given a pointer `p` to element `i` of an array of `n` elements, the expression `p + k` is defined only when `0 \<= i + k \<= n`.
In particular, forming a pointer more than one position past the end of an array object is undefined behavior -- even if the pointer is never dereferenced.
...
However, sanitizers (e.g., AddressSanitizer) and `constexpr` contexts can in principle flag the formation of such pointers

(Possible mitigations at the library level include, at a cost, over-allocating (padding the allocation so that strided `.end()` pointers remain in-bounds), using integer offset arithmetic internally and forming pointers only at the point of dereference, or representing `.end()` as a sentinel/count rather than a pointer.)

(The `multi::restriction`s are not affected by this problem because they do manipulate pointer.
