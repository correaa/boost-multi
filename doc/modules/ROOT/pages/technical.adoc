...
The main idea is that the extents of an array can be determined at runtume, for applications such as simulations, the sizes adapt to the problem sizes, or set interactively/input by the user or from a previous run.
...
Even if the sizes of some are arrays are fixed (like for a chessboard, 8 x 8), there is utility in choosing arbitrary subsets of the array for processing.
...
== Why is c++17 chosen as the minimum?

The implementation use `if constexpr` intensively, if you really need C++14 please open an issue.
