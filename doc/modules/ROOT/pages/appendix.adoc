...
| Printing                    | `print *, numbers`                               | `std::cout << numbers;` (needs `multi/io.hpp`) or  `std::print("{}", number);` (needs pass:[C++23])                  
...
| Construction 1D (3 elements) | `real*8 :: v1D(3)`   (at top)                    | `multi::array<double, 1> v1D(+{3}+);` (at scope)
...
| Indexing                    | `A[i, j]`                 | `A[i][j]` or `A(i, j)` or `A[i, j]` (C++23)
| row `i`                     | `A[i;]`                   | `A(i, _)` or `A[i]` or `A[i, _]` (C++23)
| column `j`                  | `A[;j]`                   | `A(_, j)` or `A[i, _]` (C++23)
| subrange of indices `I`     | `V[I]`                    | `V(I)`
| Compress                    | `U / V`                   | no (approx: https://godbolt.org/z/z8M6qhq5G)
| Compress last axis          | `U / A`                   | no
| Restructuring               | `,A`                      | `A.elements()`
|                             | `3 4 ⍴ V`                 | `multi::array_ref<int, 2>(V.data_elements(), {3, 4})`
|                             | `3 4 ⍴ A`                 | `multi::array_ref<int, 2>(A.data_elements(), {3, 4})`
| Catenation                  | `V, W`                    | no (approx: https://godbolt.org/z/cKcbWTooG)
| Transposition               | `⍉ A`                     | `~A` or `A.transposed()`
|                             | `1 1 ⍉ A`                 | no
| Drop                        | `3 ↓ V`                   | `V.dropped(3)`
|                             | `2 -1 ↓ A`                | `A.taked(A.size() - 1).dropped(2)`
