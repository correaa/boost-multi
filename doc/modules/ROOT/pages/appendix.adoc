...
But be aware that creating a new array to assign is inefficient, because it will allocate memory:

[source,cpp]
-----
  arr = multi::array<double, 2>(arr.extensions(), 2);  // allocates here, inefficient, not recommended
-----
