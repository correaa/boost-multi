...
...
  ...
  std::ranges::fill(arr.elements(), 0);
...
-----
...
-----
...
  std::ranges::fill(arr({1, 3}, {1, 3}).elements(), 1);
-----

If you don't have access to the ranges library (C++20) or if you want to control parallelism use:
[source,cpp]
-----
#include<algorithm>
#include<execution>
...
If you want are creating the array it is easier to just construct it with a given value.

[source,cpp]
-----
  multi::array<double, 2> arr({10, 10}, 1);  // generate a 10x10 array with all elements equal to 1
-----
