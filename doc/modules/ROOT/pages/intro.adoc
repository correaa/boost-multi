...
NOTE: This library is proposed for inclusion in Boost and is currently under Boost review, under the name *Boost.Multi*. It is already usable as a standalone, header-only library.
...
[source,plaintext]
----
 1D                       2D                     3D               plane 0      etc
    ├─0───┼─1───┼─2───┼       ┼─0──┼─1──┼─2──┼       ┼─0──┼─1──┼─2──┤ plane 1
...
Problems such as deep assignment or const-propagation are not fully solved, while ownership is delegated to a separate future proposal `std::mdarray` (C{plus}{plus}26).
...
* xref:appendix.adoc#appendix_comparison[Comparison to other array libraries] -- how _Multi_ relates to Boost.MultiArray, `std::mdspan`, Eigen, Fortran, and NumPy/APL.
