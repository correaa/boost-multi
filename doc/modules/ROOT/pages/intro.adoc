...
1D ┌─0──┬─1──┬─2──┐  2D  ┌─0──┬─1──┬─2──┐  3D  ┌─0──┬─1──┬─2──┐      4D, ... ND
   ...
   └────┴────┴────┘      ├────┼────┼────┤      ├────┼────┼────┤ u │
                         ...
                         └────┴────┴────┘      └────┴────┴────┘ x │
                                                  └────┴─────┴────┘
```
These depicted 1D, 2D, and 3D arrays can be represented, with this library, as C++ objects idependently declared as
```cpp
multi::array<char, 1> A1D = {'a', 'b', 'c'};
multi::array<char, 2> A2D = { {'f', 'g', 'h'}, {'i', 'j', 'k'} };
multi::array<char, 3> A3D = /*... a longer nested list here*/ ;
```
