...
For example, arrays can represent fields of physical values, such as temperature variations within a block of material, or a velocity fields in a fluid.
The state of game can be represented by placing pieces in a board made of squares.
...
or with the standard MDSpan proposal `std::mdspan` (accepted in C{plus}{plus}23)
...
Problems such are deep assignment, or const-propagation are not fully solved, and ownership is delegated to a separte proposal `std::mdarray` (C{plus}{plus}26).
