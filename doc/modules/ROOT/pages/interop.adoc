...
Both values are assignable, have the same element access patterns and iterator interface, and implement lexical comparisons.
...
[#interop_fmt]
== Pretty Printing with pass:[{fmt}]

NOTE: Although less customizable, C++23's `std::print` supports ranges out-of-the-box and works directly with arrays
...
[#interop_python]
== Python with pass:[cppjit]

The library works out-of-the-box via the automatic `cppjit` (formerly `cppyy`) bindings, providing a syntax similar to that of pass:[C++].
...
Here is a complete
link:https://colab.research.google.com/drive/1MbSdnRkqE9jIE0038nEQL8fRkOBEaAyC?usp=sharing[Python session]:

[source,python]
----
# We import the library
import cppjit
cppjit.add_include_path('path-to/boost-multi/include')
cppjit.include("boost/multi/array.hpp")  # or from single header https://correaa.gitlab.io/boost-multi/boost-multi.hpp

multi = cppjit.gbl.boost.multi
...
a2d = multi.array['double', 2]((2, 2), 0.0)
...
The last line reassigns the variable into a Python list, changing its Python type.
