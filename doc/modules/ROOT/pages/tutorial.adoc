...
The language limitation is that the array must no cross from compile-time to runtime context.

In C++20, if you want an array with _full_ compile-time/run-time (constexpr) valuation support, use `multi::inplace_array`.
(This is possible because this kind of array never allocates in the heap.)
