...
The language limitation is that the array (and more specifically, the memory allocation) must no cross from compile-time context to runtime context.

In C++20, if you want an array with _full_ compile-time/run-time (constexpr) valuation support, use `multi::inplace_array`.
(This is possible because it never allocates in the heap.)

[source,cpp]
----
	multi::inplace_array<int[3]> arr = {0, 1, 2};

	static_assert( arr.size() == 3 );  // or assert
	static_assert( arr[1] == 1 );  // or assert
----
