...
In many domains of computing, data is inherently multidimensional; individual elements have logical position such an certain discretized space.
...
For example, consider a two-dimensional matrix of sizes M × N stored in a buffer.
...
Reading or writting into the element at position (i, j) requires computation of the position offset in the linear buffer.

```
data[i * M + j] = 99;  // interpreted as array element (i, j)
```

This approach works for simple applications.
However there are several problems with this encoding of the multidimensional data structure.
First, the buffer `data` needs to be propagated toghether with the stride `M` for data access, (and with `N` to know the logical dimensions of the 2D array).
These are in principle separate variables.

Second, information about the order is implicit and has to be agreed upon consistently across the whole code;
in this case an equally acceptable encoding for a N x M array could be `data[i + j*M]`.
Any change in layout (column-major, padded rows, subviews, GPU memory, etc.) forces a rewrite of indexing logic and increases the risk of subtle bugs.

Third, manipulating subarrays requires the propagation of different stride layout information and a different sub-set of the buffer.

Finally, each dimension (e.g. 1D, 2D, 3D) would need to be explicitly code and the relation between subarray of lower dimensionality and the main array would be opaque.

A proper multidimensional array abstraction encapsulates shape and stride information, allowing users to write `A(i, j)` instead of manually computing offsets.
This improves readability, safety, and maintainability, while still enabling zero-overhead access and layout flexibility.
In short, a multidimensional array type expresses intent directly and separates algorithmic logic from memory layout details—something manual indexing simply cannot do cleanly at scale.

There are several libraries for multidimensional arrays in C++ (see section comparison with other libraries) and all solve this abstraction problem with different levels of generality.
Multi aims to be general in several aspects, by controlling memory, index access, subarray access, index access, iteration, etc.
...
Because this array is not controled by the library, this type of object is called an array-reference.
