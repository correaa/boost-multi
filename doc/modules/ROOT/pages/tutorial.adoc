...
Array objects (`multi::array`) own the elements they contain and can be resized later.
An `array` is initialized by specifying the index extensions and, optionally, a default value).

```cpp
multi::array<int, 1> A1({3}      , 11);  // {11, 11, 11}

multi::array<int, 2> A2({2, 3}   , 22);  // { {22, 22, 22}, {22, 22, 22} }

multi::array<int, 3> A3({3, 2, 2}, 33);  // { { { 33, ...}, { ... }, ... } }
```
...
Unlike `array` objects, `array_ref` objects do not own the elemnts they contain, and they are initialized from a preexisting contiguous range, the index extensions should be compatible with the total number of elements.
...
An elemental use for restrictions is for generating concrete arrays from coordinates, replacing common nested initialization loops:
```cpp
...
using multi::broadcast::operator+;  // import elementwise lazy addition
using multi::broadcast::operator*;  // import elementwise lazy multiplication
...
A key feature of these operations is automatic "broadcasting", which simplifies notation and is common in modern array languages.
So-called broadcasting (in Numpy or Matlab) or scalar extension (in APL) allows binary operations on data of different dimensionality.
Dimensionality mismatches are handled by broadcasting, but size mismatches are not (assertions catch these in debug mode).

In this example brew, the scalar `1` is "repeated" and interpreted as `{1, 1, 1}` (without copies) to match the left operand's dimensions.
```cpp
...
using multi::broadcast::operator+;
using multi::broadcast::operator*;
using multi::broadcast::operator/;  // import elementwise lazy division
using multi::broadcast::exp;        // import elementwise exponentiation
...
Expressions can be named (e.g., `expr` above), but they generally do not own the objects used in the expression.
...
Custom functions can be applied by using the `invoke` function:
```cpp
auto const& AVE = multi::broadcast::invoke([](auto a, auto b) { return (a + b) / 2.0 ; }, A, B);
```

Note that lazy expression are themselves multidimensional arrays, and share their generic interface (`.extensions()`, `.size()`, `.begin/end()`, etc) therefore they can be combined with algorithms.
