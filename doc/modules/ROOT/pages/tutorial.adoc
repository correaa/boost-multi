...
[#tutorial_manip]
== Manipulate your Memory with Array Refences
...
Array and element initialization can be controlled separately in the library, see a later section about element initialization.

[#tutorial_copy_assign]
== Copy your Arrays through Assignment
...
== Change the Size of your Arrays
...
[#tutorial_indexing]
== Indexing rather than Iterating

Arrays provide random access to elements or subviews.
Many algorithms on arrays are oriented to linear algebra,
and are ubiquitously implemented in terms of multidimensional index access, rather than iterators.

Although the library is fine tunned for iteration, index access provides a complementary alternatives for accessing elements.
For example `*(A.begin() + n)` and `A[n]` are equivalent
and the range defined by the pair `A.begin(), A.end()` is equivalent to `A(A.extent())` and, in turn, to `A()` (even for a multidimensional array, `D > 1`).
The syntax can be combined in arbitrary ways, for example `*A[n].begin()` is equivalent to `A[n][0]`.

[#tutorial_element_access]
== Access the Elements and Subarrays
...
A[0][0]  // is an element reference  // in C++23 A[0, 0]
----
...
The code is not particularly elegant, it is just representative a how algorithms implemented with index access can look like; many simplifications can be applied using other parts of the library.

[#tutorial_slices_and_strides]
== Slice and Stride your Arrays
...
== Avoid Pitffals of Uninitialized Elements
...
== Disallowing Mutation with Const-correctness
...
== Lazy Evaluation with Restrictions
