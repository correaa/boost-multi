...
Array references do not own memory and, just as language references, cannot be rebound (i.e. resized or "reseated") to refer to a different memory location.
...
For example, the result of `A.extension()` is itself iterable and indexable, returning a sequence with valid indices for `A`.
