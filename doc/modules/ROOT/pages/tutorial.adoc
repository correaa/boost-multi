...
This library aims to be general in several aspects, dimensionality, memory control, index access, subarray access, index access, compatibility, iteration, algorithms, etc.
...
If you want to avoid this misleading notation, and alternative usage pattern is also accepted for generating references:
...
For example, the results of `A.extension()` is it self iterable and indexable; returning a sequence with valid indices for `A`.
...
`A.extensions()` deserves special attention because manipulation of this object enables powerful constructs.
As we described earlier, it can regarded as a product type of one-dimensional extensions, 
henceforth decomposed as `auto [is, js] = A.extensions();` or as `std::get<0>(A.extensions())` and `std::get<1>(A.extensions())` (like a tuple).
Furthermore `A.extensions()` can be regarded as an n-dimensional array whose elements are the cartesian product of valid indices.
In our example:
...
[source,cpp]
----
	for(auto [i, j] : A.extensions().elements()) {
		A[i][j] = i + 10*j;
	}
----
...
----
std::transform(
	A.extensions().elements().begin(), A.extensions().elements().end(),  // source
	A.elements().begin(),  // destination
	[](auto const& ij) { auto [i, j] = ij; return i + 10*j; }  // function of coordinates
)
----

A first naive version of a 3x3 Gaussian blur applied to 3 arrays (input, output and a kernel) could be written as,
...
----
	...
	for(auto [i, j] : out.extensions().elements()) {  // go through all the indices of out
		double sum = 0.0;
		for(auto [k, l] : kernel.extensions().elements()) {  // go through all the indices of kernel
			// skip boundary elements here
			sum += in[i + k][j + l] * kernel[k][l];
		}
		out[i][j] = sum;
	}
----
link:https://godbolt.org/z/njd8cozrc[(open on Compiler Explorer)]
