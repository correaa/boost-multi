...
Arrays can be viewed simultaneously as a nested array or as flat set of elemets.
...
The elements can be also accesed through iterators; 
`A.begin()`, `A.begin()->begin()` and `A.elements().begin()` are examples of valid ways to access the elements that allows the iteraction with algorithms.
...
[source,cpp]
----
multi::array<double, 3> C({3, 4, 5});  // 60 elements with unspecified values
