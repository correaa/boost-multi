...
1D ┌─0──┬─1──┬─2──┐  2D  ┌─0──┬─1──┬─2──┐  3D  ┌─0──┬─1──┬─2──┐      4D, ... ND
   │  0 │  1 │  2 │     0│  0 │  1 │  2 │     0│  0 │  1 │  2 │2──┐
   └────┴────┴────┘      ├────┼────┼────┤      ├────┼────┼────┤ 8 │
                        1│  3 │  4 │  5 │     1│  3 │  4 │  5 │───┤
                         └────┴────┴────┘      └────┴────┴────┘11 │
                                                  └────┴─────┴────┘
```
These depicted 1D, 2D, and 3D arrays can be represented by objects declared as 
```cpp
multi::array<int, 1> A1D = ...;
multi::array<int, 2> A2D = ...;
multi::array<int, 3> A3D = ...;
```
and the elements, when retrived by their coordinates (indices) are such that, for example: `A1D[1] == 1`, `A2D[1][0] == 3` and `A3D[1][0][2] == 7`
(note that the 3D case has two planes, the first index selects the plane.)
