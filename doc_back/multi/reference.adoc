...
To use these types use `#include <boost/multi/array_ref.hpp>`.
...
To use this and derived types use `#include <boost/multi/array.hpp>`.
...
Restrictions are array that lazily generate its elements on the fly, based on some function or function object.
The do not in general allocate or manage any memory, and elements are immutable.

The interface is the same as `subarray` except that there is no `.base()` member and indexing operators return element values, rather than references.

To use this type, `#include <boost/multi/restriction.hpp>`
...
| (constructors)    | `restriction::restriction(extensions_t<D> xs, F fun)` constructs a D-dimensional lazy array. The array elements `rest[i][j]` are simply `fun(i, j)` if both indices are widthin the valid range.
| (destructor)      | implictly destroys the internal copy of `fun`
| ...               |
|===
